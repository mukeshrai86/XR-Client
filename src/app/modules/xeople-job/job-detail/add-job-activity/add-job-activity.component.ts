...
@Component({
  selector: 'app-add-job-activity',
  templateUrl: './add-job-activity.component.html',
  styleUrls: ['./add-job-activity.component.scss']
})
export class AddJobActivityComponent implements OnInit {
  ...
  @Input() emailConnection:boolean=false;
  @Input() JobId :any;
  ...
  constructor(changeDetectorRef: ChangeDetectorRef, media: MediaMatcher, private route: Router, private fb: FormBuilder, private routes: ActivatedRoute, public _sidebarService: SidebarService, private commonServiesService: CommonServiesService,
    ...
    private serviceListClass: ServiceListClass, private quickJobService: QuickJobService, public _integrationsBoardService: IntegrationsBoardService,
    private _KendoEditorImageUploaderService: KendoEditorImageUploaderService, private renderer: Renderer2) {
    let ValidatorsRequired = Validators.required
    ...
    this.myActivityForm = this.fb.group({
      Id: [],
            /* @When : 27-03-2023 @who: Bantee @Why:11238 @What : for custom validation*/
      slotDate: [null,[Validators.required, CustomValidatorService.dateValidator]], 
      timePeriod: ['60', Validators.required],
      ActivityTitle: ['', [Validators.required, Validators.maxLength(100)]],/** @When: 27-09-2023 @Who:maneesh for increes charecter length @Why: Ewm-14198 */
      RelatedUserType: [null],
      RelatedUserTypeName: [''],
      RelatedUserId: [null],
      RelatedUserUserName: [''],
      CategoryId: [null],/**@When: 20-03-2023 @Who:Nitin Bhati @Why: EWM-11254 EWM-11261 @What: required removed */
      CategoryName: [''],
    ...
    //////User Dropdown//////////////
    this.dropDownRelatedUserConfig['IsDisabled'] = true;
  ...
  ngOnInit(): void {
      /*** @When: 12-05-2023 @Who:maneesh @Why: EWM-9383 @What:make defualt access public **/
    ...
    let slotDate =  new Date(startdatetime.getFullYear(), startdatetime.getMonth(), startdatetime.getDate(), startdatetime.getHours(), startdatetime.getMinutes() - startdatetime.getTimezoneOffset()).toISOString().slice(0,10);
            /* @When : 27-03-2023 @who: Bantee @Why:11238 */
  ...
  */
  ...
  openModelForSchedule() {
    const dialogRef = this.dialog.open(ScheduleComponent, {
      maxWidth: "650px",
      data: new Object({ scheduleData: this.scheduleData24Hrs, activityActionForm:this.activityActionForm  }), /*--@Who: Nitin Bhati,@When:12-03-2023,@Why: EWM-10638,@What:For passing parameter for schedular time 24--*/
      width: "80%",
      maxHeight: "85%",
    ...
    this.quickJobService.fetchUserInviteListForOrganization().subscribe(
      (repsonsedata: any) => {
        if (repsonsedata.HttpStatusCode === 200 || repsonsedata.HttpStatusCode === 204) {
           ...
           this.myActivityForm.controls['OrganizerOrAssignees'].setValue(repsonsedata.Data.filter((e: any) => e.UserId === userId));
           this.organizerOrAssigneesList= 
           repsonsedata.Data.map((p) =>
            p.UserId!== userId ? p : { ...p, disabled: true }
            );
          
            this.selectedOrganizer = repsonsedata.Data.filter((e: any) => e.UserId === userId);
        } else {
          this.snackBService.showErrorSnackBar(this.translateService.instant(repsonsedata.Message), repsonsedata.HttpStatusCode);
        }
      }, err => {
  ...
  createActivity(value) {
   ...
   this.scheduleTimeData.TimeZoneID =  value.TimeZoneID;
    ...
    myActivityJson["Location"] = value.Location;
     ...
     myActivityJson["OptionalAttendeesList"] =this.optionalAttendeesList
 ...
 */
  ...
  onDismiss(): void {
    this.action = false;
    //this.sidenav.close();
    this.clientActivityCount.emit();
    this.closeDrawer.emit(true); // @suika drawer closer @whn 21-03-2023
    this.requiredAttendeesList = [];
    this.organizerOrAssigneesList = [];
    this.scheduleData = [];
    this.oldPatchValues = {};
    ...
    updateActivityJson["ReplaceTag"]=ReplaceTagArr;
      ...
      if(this.requiredAttendeesList.length!==0){
        this.requiredAttendeesList.forEach(element => {
         element['IsOptional']=0;
        });
      }
     if(this.optionalAttendeesList.length!==0){
       this.optionalAttendeesList.forEach(element => {
         element['IsOptional']=1;
        });
     }
      
     updateActivityJson["OptionalAttendeesList"] = this.optionalAttendeesList;
 ...
 if (res.OrganizersList != '') {
  ...
  this.myActivityForm.patchValue({
   ...
   p.UserId!== this.userId ? p : { ...p, disabled: true }
            ...
            this.getScheduleAssitance(res.ScheduleActivity?.DateStartUTC,res.ScheduleActivity?.DateEndUTC);
            /* @When : 21-09-2022 @who: Renu @Why:1734 EWM-8251 @Waht : for scehdule assitance*/
          
            this.onChangeActivityRelatedTo(res.RelatedUserType);
              if(res.MeetingPlatformId){
              this.readonly=true;
             }else{
              this.readonly=false;
             }
...
*/
  previewEmailPopUp() {
    ...
    data: new Object({ subjectName: this.myActivityForm.get("ActivityTitle").value, emailTemplateData: this.myActivityForm.get("Description").value,RelatedUserType: this.category,RelatedUserId: this.candidateIdData,CandidateId:this.candidateId,isActivity:true,JobId:this.JobId,openDocumentPopUpFor:"JOB"}),
