...
@Component({
  selector: 'app-job-call-histroy',
  templateUrl: './job-call-histroy.component.html',
  styleUrls: ['./job-call-histroy.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class JobCallHistroyComponent implements OnInit {
  ...
  @Input() JobIdForVxt:string;
  JobdetailsSummeryUrl:string;
  ...
  constructor(public dialog: MatDialog, private clientService: ClientService, private jobService: JobService,
    ...
    this.getCallLogData = JSON.parse(sessionStorage.getItem('jobDetails'));               
    this.candidateName = this.getCallLogData?.JobTitle;        
    this.candidateId = this.getCallLogData?.Id;
    // this.StatusColorCode = this.getCallLogData?.HeaderDetails?.JobDetails?.Id
    // this.ShortName = this.getCallLogData?.HeaderDetails?.JobDetails?.Id
    // this.ImageUrl = this.getCallLogData?.HeaderDetails?.JobDetails?.Id
    // this.PhoneNumber = this.getCallLogData?.HeaderDetails?.JobDetails?.Id
    // if (this.JobIdForVxt!=undefined) {
    //   this.candidateId=this.JobIdForVxt;
    // }else{
    //   this.candidateId=this.JobIdForVxt;
    // }
  ...
  clearFilterDates(): void {
    ...
    const title = '';
    const subTitle = ''; // Call remove from subtitle
    const dialogData = new ConfirmDialogModel(title, subTitle, message);
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      maxWidth: "350px",
      data: dialogData,
  ...
  public sendRequest(state: State): void {
    ...
    if (data!=null && data!=undefined) { //by maneesh
      this.candidateId =data?.JobDetails?.Id;  
      this.commonserviceService.VxtCountApiCall.next(this.candidateId);
      }    
    this.animate();
  ...
  OpenDocuemntPopUp(add) {
    ...
    this.detailsSummery = '/client/jobs/job/job-list/list/';
    this.JobdetailsSummeryUrl = baseUrl + this.detailsSummery + this.candidateId + "&filter=" + 'TotalActiveJobs';
    const dialogRef = this.dialog.open(AddJobCallComponent, {
      data: new Object({
        ShortName: this.ShortName, Name: this.candidateName, Id: this.candidateId, StatusColorCode: this.StatusColorCode,
        ImageUrl: this.ImageUrl, usertype: "JOB", isEdit: add, gridlistData: this.gridListData,
        ToNameredirectUrl: this.tONameRedirectUrl,reletedUserTypeCodeValue:'Job',reletedUserTypeCode:'JOB',
        FromNameredirectUrl: this.fromRedirectUrl, detailsSummeryUrl: this.detailsSummeryUrl,JobIdForVxt:true,
        JobdetailsSummeryUrl: this.JobdetailsSummeryUrl
      }),
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      ...
      if (dialogResult == true) {
        ...
        this.loading = true;
        this.getFilteredConfig();
        this.commonserviceService.countDataRefreshForContact.next({jobCall:true,jobId:this.candidateId});
        // this.sendRequest(this.state)
  ...
  openEditCallPopup(dataItem) {
    ...
    const dialogRef = this.dialog.open(AddJobCallComponent, {
      data: new Object({
        ShortName: this.ShortName, Name: this.candidateName, Id: this.candidateId, StatusColorCode: this.StatusColorCode,
        ImageUrl: this.ImageUrl, dataItem: dataItem, callId: dataItem?.callId, isEdit: 'Edit', usertype: "JOB",
        reletedUserTypeCodeValue:'Job',reletedUserTypeCode:'JOB',JobIdForVxt:true,
        JobdetailsSummeryUrl: this.JobdetailsSummeryUrl
