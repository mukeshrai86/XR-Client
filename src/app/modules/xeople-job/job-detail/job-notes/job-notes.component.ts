/*
  ...
  @What:  This page will be use for job Notes
*/
...
import { XRNotifications,NotifyUser } from '@app/shared/models/notes';
import { MentionEditorComponent } from '@app/shared/mention-editor/mention-editor.component';
import { SearchNoteByContactComponent } from '@app/modules/EWM.core/client/client-notes/search-note-by-contact/search-note-by-contact.component';
import { SearchJobNoteByContactComponent } from './search-job-note-by-contact/search-job-note-by-contact.component';
...
export class JobNotesComponent implements OnInit {
 ...
 @Output() jobNotesCount = new EventEmitter();
 @Input() jobIdData: any;
...
//  kendo image uploader Adarsh singh 31-july-2023
...
public selectedorDeselectedOwner:[];
  //Who:Ankit Rawat, What:EWM-EWM-16073-EWM-16200 Add new control Contact Dropdown, When:23Feb24
  ...
  constructor(changeDetectorRef: ChangeDetectorRef, media: MediaMatcher,private fb: FormBuilder,private routes: ActivatedRoute, public _sidebarService: SidebarService, private commonServiesService: CommonServiesService,
    ...
    private jobService: JobService, private snackBService: SnackBarService, private clientService: ClientService,public candidateService: CandidateService,
    private serviceListClass: ServiceListClass, private _KendoEditorImageUploaderService: KendoEditorImageUploaderService,
    private renderer: Renderer2,private route: Router,private cache : CacheServiceService) {
      // who:maneesh,what:ewm-11655 for access name value PathLocationStrategy,when:17/04/2023
    ...
    this.clientNotesForm = this.fb.group({
      ...
      NotesDate: [null,[Validators.required, CustomValidatorService.dateValidator]],
      CategoryId: [''],
      ...
      file: [''],
      AccessId: [],
      // who:maneesh,what:ewm-11655 for access name value ,when:17/04/2023
  ...
  ngOnInit(): void {
    ...
    this.searchNotes= this.searchSubject$.pipe(debounceTime(1000)).subscribe(value => {
      ...
      this.getJobNotesList(this.clientIdData, this.yearFilterRes, this.monthFilterRes, this.pagneNo,value);
      this.getJobNotesYearMonthList(); 
    });
    ...
    localStorage.removeItem('JobNotes_grid_001');
    this.getFilterConfig(true);
    this.getJobNotesYearMonthList();
    this.getJobNotesList(this.clientIdData, this.yearFilterRes, this.monthFilterRes, this.pagneNo,this.searchValue);
   // this.getNoteCateogoryList();
             // <!---------@When: 19-07-2023 @who:Bantee @why: EWM-13029 What:When user adds new notes category via Manage icon, then added category is not displaying on category dropdown list --------->

   this.dropDoneConfig['IsDisabled'] = false;
   this.dropDoneConfig['apiEndPoint'] = this.serviceListClass.notesCategoryList+'?UserType=' +'Jobs' +'&FilterParams.ColumnName=StatusName&FilterParams.ColumnType=DropDown&FilterParams.FilterValue=active&FilterParams.FilterOption=IsEqualTo' ;
  ...
  ngAfterViewInit() {
    this.routes.queryParams.subscribe((value) => {
      if (value?.uniqueId !=undefined && value.uniqueId!=null && value.uniqueId!='') {
        ...
        this.jobIdData= value?.jobId;
        this.formHeading = 'View';
        this.sidenav.toggle();
        this.gridListView=[];
        if(this.clientIdData!=value?.jobId){
          this.clientIdData = value?.jobId;
          this.getFilterConfig(true);
          this.getJobNotesYearMonthList();
        }
        this.viewNotesById(this.uniqueId,this.formHeading,this.jobIdData); 
       }
    });
  }
  viewNotesById(Id: number,formtype:string,jobIdData:string) {
    this.formHeading = formtype;
    this.loading = true;
    this.jobService.getRedirectedJobNotebyid('?JobId='+jobIdData+'&NotesId='+Id)
    .subscribe(
      (data: ResponceData) => {
        this.route.navigate([],{
  ...
  */
  ...
  openFilterModalDialog() {
    const dialogRef = this.dialog.open(FilterDialogComponent, {
      data: new Object({ filterObj: this.filterConfig, GridId: 'JobNotes_grid_001' }),
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res != false) {
        ...
        this.loading = true;
        let jsonObjFilter = {};
        jsonObjFilter['JobId'] = this.jobIdData;
        ...
        this.jobService.fetchJobNotesAll(jsonObjFilter).subscribe(
  ...
  */
  ...
  getJobNotesYearMonthList() {
    let jsonObj = {};
    jsonObj['JobId'] = this.jobIdData;
    if (this.OwnerIds) {
    ...
    this.jobService.fetchJobNotesMonthYearCountAll(jsonObj)
      .subscribe(
        (data: ResponceData) => {
          if (data.HttpStatusCode === 200) {
 ...
 */
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      ...
      if (dialogResult == true) {
       ...
       // jsonObj['OrderBy'] = '';
        ...
        jsonObjFilterG['GridConfig'] = this.colArr;
        this.jobService.fetchJobNotesAll(jsonObj).subscribe(
          (repsonsedata: ResponceData) => {
            if (repsonsedata.HttpStatusCode === 200 || repsonsedata.HttpStatusCode === 204) {
              this.loading = false;
              this.gridList = repsonsedata.Data;
              ...
              if (viewMode == 'Category') {
                ...
                this.selectedOrDeSelected?.forEach(ele=>{
                 ...
                 sessionStorage.removeItem('SelectedJobNotsCatogry')
  ...
  getJobNotesList(clientId: any, year: number, month: string, pageNo,searchValue) {
    ...
    //jsonObj['NotesFilterParams'] = this.filterConfig;
     //<!-- who:Nitin Bhtai,why:EWM-14913,when:26-10-2023 -->
     if (this.filterConfig !== undefined && this.filterConfig !== null && this.filterConfig !== '') {
      jsonObj['NotesFilterParams'] = this.filterConfig;
    } else {
      jsonObj['NotesFilterParams'] = [];
    ...
    jsonObj['FromDate'] = this.FromDate ? this.FromDate : null;
    jsonObj['ToDate'] = this.ToDate ? this.ToDate : null;
    this.jobService.fetchJobNotesAll(jsonObj)
      .subscribe(
        (data: ResponceData) => {
          if (data.HttpStatusCode === 200) {
            ...
            this.loading = false;
            //this.gridListView = this.gridList?.filter(x => x['Id'] == this.uniqueId);
          }
          else if (data.HttpStatusCode === 204) {
            this.gridList = data.Data;
            this.gridListData = data.Data;
  ...
  */
  ...
  public onMessage(value: any) {
    if (value != undefined && value != null) {
      this.maxMessage = 200 - value.length;
      if(value?.length>200){
        this.clientNotesForm.get('NoteTitle').markAsTouched();
        this.clientNotesForm.get('NoteTitle').setErrors({maxlength:true});
      }
    }
  }
...
*/
  updateJobNotes(value) {
    ...
    let XRNotificationsArr:XRNotifications={
      ...
      NavigateUrl:'client/jobs/job/job-detail/detail?jobId='+this.jobIdData+'&workflowId='+this.workflowId+'&filter=All&selectjob=TotalJobs&tabIndex=3&uniqueId=',
      CompletePath:window.location.href+'&uniqueId='
    };
    updateNotesObj['JobId'] = this.jobIdData;
    const d = new Date(value.NotesDate);
    updateNotesObj['NotesDate'] =  new Date(d.getFullYear(), d.getMonth(), d.getDate(), d.getHours(), d.getMinutes()).toISOString();
    updateNotesObj['CategoryId'] = this.CategoryId;
    updateNotesObj['NoteTitle'] = value.NoteTitle?.trim();
    updateNotesObj['Subject'] = value.NoteTitle?.trim();
    updateNotesObj['Description'] = value.Description === null ? '':value.Description;
    updateNotesObj['Attachment'] = this.filePath;
    updateNotesObj['AttachmentName'] =this.uploadedFileName;
    updateNotesObj['AccessId'] = value.AccessId;
    updateNotesObj['Id'] = value.Id;
    updateNotesObj['GrantAccesList'] = this.accessEmailId;
    updateNotesObj['NotesURL'] = window.location.href;
    updateNotesObj['Files'] = files;
    updateNotesObj['XRNotifications']=XRNotificationsArr;
    updateNotesObj['CallReferenceId']=this.CallReferenceId;
    // who:maneesh,what:ewm-12708 for pass UserTypeCode when:05/06/2023
    let typeCode = this.GridId=== 'JobNotes_grid_001' ? 'JOB' : '';
    updateNotesObj['UserTypeCode'] = typeCode;
    updateObj = [{
      "From": fromObj,
      "To": updateNotesObj
    }];

       //Who:Ankit Rawat, What:EWM-16200 EWM-16200 mapped Contacts with Job Note, When:26 Feb 2024
       if(this.selectedContactsItems?.length>0){
        updateNotesObj['NotesContacts']=this.selectedContactsItems?.map(item => ({
          'ContactId':item.Id,
          'FirstName':item.FirstName,
          'LastName':item.LastName,
          'Email':item.Email,
          'ProfileUrl':item.PreviewUrl
        }));
      }
    this.jobService.EditJobNotesAll(updateObj[0])
      .subscribe(
        (data: ResponceData) => {
          if (data.HttpStatusCode === 200) {
            this.loading = false;
            ...
            this.fileAttachments=[];
             ...
             // <!---------@When: 02-12-2022 @who:Adarsh singh @why: EWM-9142 --------->
             this.isDropdownShow = false;
    // <!-- // who:bantee,what:ewm-14049 ,when:27/08/2023 Notesattachment issue -->
...
*/
  createJobNotes(value) {
    ...
    let AddNotesObj = {};
    AddNotesObj['JobId'] = this.jobIdData;
    const d = new Date(value.NotesDate);
    AddNotesObj['NotesDate'] =  new Date(d.getFullYear(), d.getMonth(), d.getDate(), d.getHours(), d.getMinutes()).toISOString();
    AddNotesObj['CategoryId'] = this.CategoryId;
    AddNotesObj['NoteTitle'] = value.NoteTitle?.trim();
    AddNotesObj['Subject'] = value.NoteTitle?.trim();
    AddNotesObj['Description'] = value.Description === null ? '':value.Description;
    AddNotesObj['Attachment'] = this.filePath;
    AddNotesObj['AttachmentName'] =this.uploadedFileName;
    AddNotesObj['AccessId'] = value.AccessId;
    AddNotesObj['GrantAccesList'] = this.accessEmailId;
    AddNotesObj['NotesURL'] = window.location.href;
    AddNotesObj['Files'] = files;
    AddNotesObj['XRNotifications'] = XRNotificationsArr;
   
    //Who:Ankit Rawat, What:EWM-16200 EWM-16200 mapped Contacts with Job Note, When:26 Feb 2024
    if(this.selectedContactsItems?.length>0){
      AddNotesObj['NotesContacts']=this.selectedContactsItems?.map(item => ({
        'ContactId':item.Id,
        'FirstName':item.FirstName,
        'LastName':item.LastName,
        'Email':item.Email,
    ...
    // who:maneesh,what:ewm-12708 for pass UserTypeCode when:05/06/2023
    let typeCode = this.GridId=== 'JobNotes_grid_001' ? 'JOB' : '';
    AddNotesObj['UserTypeCode'] = typeCode;
...
*/
  editClientForm(Id:number,formtype:string,CallReferenceId) {
   this.CallReferenceId=CallReferenceId
    ...
    this.jobService.getJobNotesById('?Id=' + Id)
      .subscribe(
        (data: ResponceData) => {
          if (data.HttpStatusCode === 200) {
            ...
            // @Who: Ankit Rawat, @When: 26-Feb-2024, @Why: EWM-16073 EWM-16200 (set contacts value to view selected items in edit mode)
            ...
            this.editorConfig.DESC_VALUE= res.Description;
  ...
  */
  ...
  /*
  @Type: File, <ts>
  @Name: deleteJobNotes function
  @Who: Suika
  @When: 13-April-2022
  @Why: EWM-4668 EWM-6153
  @What: call Api for delete record .
  */
  deleteJobNotes(val): void {
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      if (dialogResult == true) {
        ...
        delObj = val;
        this.jobService.deleteJobNotesById(delObj).subscribe(
          (repsonsedata: any) => {
      ...
      (repsonsedata: ResponceData) => {
        if (repsonsedata.HttpStatusCode === 200 || repsonsedata.HttpStatusCode === 204) {
          ...
          if (repsonsedata.Data !== null) {
            ...
            this.filterConfig = repsonsedata.Data?.FilterConfig;//who:maneesh,what:ewm-14913 for handel null,when:25/10/2023
 ...
 */
  openDialogJobCategory() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result!=false ) {
        sessionStorage.setItem('SelectedJobNotsCatogry',JSON.stringify(result.selectedOrDeSelected) );
        ...
        this.selectedOrDeSelected=JSON.parse(sessionStorage.getItem('SelectedJobNotsCatogry'));
  ...
  onScrollDown() {
    ...
    if (this.canLoad) {
      ...
      if (this.totalDataCount > this.gridList.length) {
        ...
        this.getJobNotesListScroll(this.clientIdData, this.yearFilterRes, this.monthFilterRes, this.pagneNo);
      } else {
  ...
  getJobNotesListScroll(clientId: any, year: number, month: string, pageNo) {
    this.loading = true;
    this.yearFilterRes = year;
    this.monthFilterRes = month;
    let jsonObj = {};
...
formType(value) {
  this.clientNotesForm.enable();
  this.ShowEditor=true;
  // @Who: Ankit Rawat, @When: 27-Feb-2024,@Why: EWM-16073-EWM-16200 (removed selected items from contact dropdown)
  this.selectedContactsItems=[];
  // who:maneesh,what:ewm-11655 for by default public button patch,when:18/04/2023
  this.clientNotesForm.patchValue({
 ...
 if (value=='Add') {  //who:maneesh,what:ewm-15285 for handel save enabel disabel,when:07/12/2023
  ...
  this.getEditorVal= ''; //clear data when add by maneesh ,when:21/04/2024
}
...
dropdownConfigContacts(){
  ...
  this.ContactsDropdownConfig = {
    API: this.serviceListClass.getContactByClientId +'?ClientId='+this.InputClientID,
    MANAGE: '',
    BINDBY: 'Name',
    REQUIRED: false,
    DISABLED: false,
   ...
   //  @Who: Renu, @When: 27-Feb-2024,@Why: EWM-15213-EWM-16109 @What: on changes on kendo editor catch the event here
 ...
 if(event && event?.val && event?.val?.replace(/(<([^>]+)>)/ig,"")?.length!==0){
  ...
  //  @Who: Renu, @When: 27-Feb-2024,@Why: EWM-15213-EWM-16109 @What:get all mentions info from html tags on save/update
  ...
  //  @Who: Renu, @When: 27-Feb-2024,@Why: EWM-16207 EWM-16299 @What: get all event for editor here
  ...
  openDialogSearchByContact() {
    const dialogRef = this.dialog.open(SearchJobNoteByContactComponent, {
      data: { selectedorDeselected: this.selectedorDeselectedContact,clientId :this.InputClientID },
      panelClass: ['xeople-modal', 'add_teamMate', 'animate__animated', 'animate__zoomIn'],
      disableClose: true,
    });
    dialogRef.afterClosed().subscribe(result => {
  ...
  getClientJobCount() {
    this.clientService.fetchContactCount("?clientId="+this.InputClientID).subscribe(
      (repsonsedata: ResponceData) => {
        if (repsonsedata.HttpStatusCode === 200) {
          this.loading = false;
