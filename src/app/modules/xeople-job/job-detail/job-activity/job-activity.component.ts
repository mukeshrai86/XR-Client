  ...
  ngOnInit(): void {
   ...
   // let slotDates = this.datePipe?.transform(slotDate,this.dateFormat);
    ...
    if(this.emailConnection){
      this.myActivityForm.get('IsSendCalendarInviteToAttendees').setValue(1);
     }else{
       ...
       if(this.requiredAttendeesList?.length==0 && this.optionalAttendeesList?.length==0){
        ...
        //who:maneesh,what:ewm-16340 new speech editor,when:26/03/2024 -->
  this.editorConfig={
    REQUIRED:false,
    DESC_VALUE:null,
  ...
  */
  ...
  filterData: any;
    ...
    getAllInviteUser() {
      ...
      let regionId = this.timezoneArrList.find(x => x['Id'] == userTimeZone);
      this.myActivityForm.patchValue({
        TimeZoneID: regionId?.Id
      })
 ...
 */
  ...
  getActivityList(clientId: any, year: number, month: string, pageNo,searchVal) {
        ...
        (repsonsedata:ResponceData) => {
          ...
          this.loading = false;
           if (repsonsedata.HttpStatusCode === 200) {   
            ...
            // this.getEditorVal=  repsonsedata['Data'].TemplateText; //patch data when edit by maneesh ,when:26/03/2024
...
*/
  patchScheduleData(value) {  
    ...
    let timeZoneData:any;
     ...
     let DateStart = value.DateStartUTC;
...
*/
  previewEmailPopUp() {
    ...
    data: new Object({ subjectName: this.myActivityForm.get("ActivityTitle").value, emailTemplateData: this.myActivityForm.get("Description").value,RelatedUserType:this.category,RelatedUserId:this.candidateIdData,CandidateId:this.candidateId,isActivity:true,openDocumentPopUpFor:"Candidate"}),
...
//by maneeesh,when:01/05/2024,for search data
