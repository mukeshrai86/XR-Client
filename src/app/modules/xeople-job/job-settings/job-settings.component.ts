  ...
  renderMenuData(groupdata) {
    ...
    this.menuData.forEach(element => {
      if (element.Children.length != 0) {
        activeSubmenuChildren = this.findByMatchingProperties(element.Children, { Name: "job-setting" });
        if (activeSubmenuChildren.length != 0) {
          this.submenu.push(activeSubmenuChildren);
          activeThirdLevel = this.findByMatchingProperties(activeSubmenuChildren[0]['Children'], { Name: "job" });
        }
      }
    });

    this.submenu.forEach(element => {
     // console.log("distinctname "+JSON.stringify(element));
