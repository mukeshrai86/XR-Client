...
import { SeekPreview, OfferedRemunerationPackage, RangesEntity, MinimumAmount, MaximumAmount } from '../job-add-details/seekinterface';
import { SeekPreviewUrl, OfferedRemunerationPackageUrl, RangesEntityUrl, MinimumAmountUrl, MaximumAmountUrl, PostingInstructionsEntity, PositionFormattedDescriptionsEntity, SeekVideo } from '../job-add-details/seek-preview-interface';
import { debounceTime } from 'rxjs/operators';

@Component({
  selector: 'app-seek-update-details',
  templateUrl: './seek-update-details.component.html',
  styleUrls: ['./seek-update-details.component.scss']
})
export class SeekUpdateDetailsComponent implements OnInit {
  ...
  SeekAdvertisementProductIdPreview: any;
  selectedAdTypeId: string;
  JobadCostCurrency: any;
  JobAdCost: any;
  AdvertisementProductId: any;
  previousJobAd: any;
    ...
    this.jobService.getPostedJobdetailsByid('?JobId=' + this.jobId + '&JobBoard=' + 'seek').subscribe(
      (data: ResponceData) => {
        ...
        if (data.HttpStatusCode === 200) {
          ...
          this.searchUserName=data?.Data?.ContactPerson;
          this.onsearchTeammate(data?.Data?.ContactPerson);
          this.DateExpiry = new Date(data?.Data?.JobExpiry),
            this.workTypeValue = data?.Data?.WorkType;
          this.payTypeValue = data?.Data?.PayType;
          this.payTypeIntervalCode = data?.Data?.PayTypeCode;
          this.previousJobAd = data?.Data?.JobAd;
          ...
          if (data?.Data?.AdvertisementProductId && data?.Data?.AdvertisementProductId == '') {
            this.getSeekAdselectionV2();
            //this.getSeekAdselectionV2Preview();
          } else {
            this.SeekAdvertisementProductId = data?.Data?.AdvertisementProductId;
            //this.getSeekAdselectionV2Preview();
          }
          /**@when: 15-05-2024 @why: EWM-16917 EWM-17084 @who:Renu @What: AdvertisementProductId & ProductPriceSummary added**/
          this.ProductPriceSummary = data?.Data?.ProductPriceSummary; /**@when: 15-05-2024 @why: EWM-16917 EWM-17084 @who:Renu @What:  ProductPriceSummary added**/
...
*/
  onCurrentPayAdDetails(jobAdId, jobAdTypeList) {
    if (jobAdId == 'Classic') {
      this.divStatusKey = false;
      this.divStatusBranding=false;
      this.divStatusBrandingLogo=false;
      ...
      this.SeekAccountUserObs = this._SystemSettingService.getSeekAccountUser("?Search=" + inputValue + "&OrganizationId=" + this.orgId).subscribe(
        (repsonsedata: any) => {
          if (repsonsedata?.HttpStatusCode === 200 || repsonsedata?.HttpStatusCode === 204) {
            ...
            this.userContactsList = this.searchListUser?.filter((e: any) => e?.UserName === this.searchUserName);
...
*/
  public onSave(value) {
    ...
    this.jobPublishArray['DefaultJobDetail'] = {
      ...
      this.jobPublishArray['JobAdCost'] = this.JobAdPrice; // value.SalaryCurrency;  //this.JobAdPrice;  /**WHO: Renu @WHEN:14-05-2024 @WHY:EWM-16917 EWM-17059 @WHAT: JobAdPrice & JobAdCurrency changes & added two new fields ProductPriceSummary,seekAdvertisementProductId & currency commented**/
    ...
    if (this.previousJobAd == value?.jobAdType) {
      this.jobPublishArray['IsAdvertisementProductIdChanged'] = 0;
    } else {
      this.jobPublishArray['IsAdvertisementProductIdChanged'] = 1;
    }
    ...
    const d = new Date(this.DateExpiry);
 ...
 */
  ...
  */
  ...
  getSeekAdselectionV2() {
    if (this.previousJobAd != this.JobAd) {
      //this.SeekAdvertisementProductId = null;
    this.jobAdTypeList=[];
    this.brandingId = null;
    this.addForm.get("jobAdType").reset();
    }
    // else{
    //   this.previousJobAd=null;
    // }
    ...
    this._SystemSettingService.getSeekAdselectionUpdateV2(updateObj[0]).subscribe(
      (repsonsedata: ResponceData) => {
        if (repsonsedata.HttpStatusCode == '200') {
          ...
          let jobAdTypeListV2 = this.jobAdTypeList?.filter((x: any) => x?.label == this.JobAdId);
          this.onCurrentPayAdDetails(jobAdTypeListV2[0]?.label, jobAdTypeListV2[0]);
          this.onSave(this.addForm.getRawValue());
        } else {
        }
      }, err => {
  ...
  public async createRequest(value) {
    ...
    PostingInstructionsEntityArr.push({
      seekAdvertisementProductId: this.SeekAdvertisementProductIdPreview,
    ...
    if (this.SeekAdvertisementProductIdPreview != 'undefined' && this.SeekAdvertisementProductIdPreview != undefined) {
  ...
  getSeekAdselectionV2Preview() {
    ...
    this._SystemSettingService.getSeekAdselectionV2(updateObj[0]).subscribe(
      (repsonsedata: ResponceData) => {
        if (repsonsedata.HttpStatusCode == '200') {
          let jobAdTypeList = repsonsedata?.Data?.advertisementProducts?.products;
          let jobAdTypeListPreview = jobAdTypeList?.filter((x: any) => x?.label == this.JobAd);
          if(jobAdTypeListPreview?.length=='0'){
            jobAdTypeList?.forEach(y => {
              if(y?.label=='Basic' && this.JobAd=='Classic'){
                let jobAdTypeListPreview = jobAdTypeList?.filter((x: any) => x?.label == 'Basic');
                this.SeekAdvertisementProductIdPreview = jobAdTypeListPreview[0]?.id?.value;
              }else if(y?.label=='Classic' && this.JobAd=='Basic'){
                let jobAdTypeListPreview = jobAdTypeList?.filter((x: any) => x?.label == 'Classic');
                this.SeekAdvertisementProductIdPreview = jobAdTypeListPreview[0]?.id?.value;
              }
              if(y?.label=='Branded' && this.JobAd=='StandOut'){
                let jobAdTypeListPreview = jobAdTypeList?.filter((x: any) => x?.label == 'Branded');
                this.SeekAdvertisementProductIdPreview = jobAdTypeListPreview[0]?.id?.value;
              }else if(y?.label=='StandOut' && this.JobAd=='Branded'){
                let jobAdTypeListPreview = jobAdTypeList?.filter((x: any) => x?.label == 'StandOut');
                this.SeekAdvertisementProductIdPreview = jobAdTypeListPreview[0]?.id?.value;
              }
            });
          }else{
      ...
      (repsonsedata: ResponceData) => {
        if (repsonsedata.HttpStatusCode === 200 || repsonsedata.HttpStatusCode === 204) {
          ...
          if (repsonsedata.Data) {
            ...
            this.payTypeValue = SeekPayTypeListPatch[0]?.Code;
            this.payTypeName = SeekPayTypeListPatch[0]?.Name;
            this.payTypeIntervalCode = SeekPayTypeListPatch[0]?.IntervalCode;
          }
        } else {
          this.loading = false;
          this.snackBService.showErrorSnackBar(this.translateService.instant(repsonsedata.Message), repsonsedata.HttpStatusCode);
