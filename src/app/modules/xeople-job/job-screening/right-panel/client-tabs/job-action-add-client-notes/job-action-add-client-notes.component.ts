...
@Component({
  selector: 'app-job-action-add-client-notes',
  templateUrl: './job-action-add-client-notes.component.html',
  styleUrls: ['./job-action-add-client-notes.component.scss']
})
export class JobActionAddClientNotesComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<JobActionAddClientNotesComponent>, private appSettingsService: AppSettingsService, private fb: FormBuilder, private _KendoEditorImageUploaderService: KendoEditorImageUploaderService,
...
ngOnDestroy() {
  ...
  // who:maneesh,what:ewm-13875 for open image upload Popup,when:06/09/2023
  ...
  // who:maneesh,what:ewm-13875 for open Manage Access Modal,when:06/09/2023
  ...
  // who:maneesh,what:ewm-13875 for  on validating file onMessage function,when:06/09/2023
  public onMessage(value: string) {
    if (value != undefined && value != null) {
      this.maxMessage = 200 - value?.length;
    }
  }
  // who:maneesh,what:ewm-13875 for  when category drop down changes  onCategorychange function,when:06/09/2023
  ...
  // who:maneesh,what:ewm-13875 for to close the pop up   onDismiss function,when:06/09/2023
  onDismiss() {
    this.EditorEventCapture();
  ...
  createNotes(value) {
    ...
    this.mentionList?.filter(item=>{
     ...
     let XRNotificationsArr:XRNotifications={
       ...
       NavigateUrl:'client/core/clients/list/client-detail?clientId='+ this.clientId+'&cliTabIndex=6&uniqueId=',
      CompletePath:window.location.origin+'/client/core/clients/list/client-detail?clientId='+ this.clientId+'&cliTabIndex=6&uniqueId='
     };
...
} else if (data?.checked && type == 'jobId') {
...
} else if (data?.checked == false && type == 'jobId' && this.JobId==Id) {
