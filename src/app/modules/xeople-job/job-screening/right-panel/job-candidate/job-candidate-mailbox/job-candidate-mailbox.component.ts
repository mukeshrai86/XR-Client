...
@Component({
  selector: 'app-job-candidate-mailbox',
  templateUrl: './job-candidate-mailbox.component.html',
  styleUrls: ['./job-candidate-mailbox.component.scss']
})
export class JobCandidateMailboxComponent implements OnInit {
  ...
  dataMailServiceSubscription: Subscription;
  @Input() jobId: string;
  candidateid: string;
  selectedCandidates=[];
     ...
     URL_AS_LIST = URL.substring(0, URL.indexOf("?")).split('/');
      ...
      this.dataSubscription =  this.commonserviceService.getJobScreeningInfo().subscribe((res: JobScreening) => {
        ...
        if (candidateList?.length>0) {
          this.selectedCandidates = candidateList;
          this.candidateid = candidateList[0]?.CandidateId;
          this.candidateEmail = candidateList[0]?.EmailId;
  ...
  */
  openNewEmailModal(responseData:any,mailRespondType:string) {
    ...
    const dialogRef = this.dialog.open(NewEmailComponent, {
      ...
      maxHeight: "100%",
      height: "100%",
      //who:maneesh,what:add JobTitle,when:18/03/2024
      data:{'res':responseData,'mailRespondType':mailRespondType,'openType':localStorage.getItem('emailConnection'),'candidateMail':this.candidateEmail,
         'JobId': this.jobId,'candidateId': this.candidateid,openDocumentPopUpFor:'Candidate','jobActionRightPannelMail':true,
         'candidateres': this.selectedCandidates[0],'JobTitle': this.selectedCandidates[0]?.JobTitle},
...
/* 
  ...
  */
  fetchInboxList(pagesize, pagneNo, sortingValue, searchVal,Imp) {  
    ...
    this.loading = true;
    this.dataMailServiceSubscription =  this.mailService.fetchCandidateMailList(pagesize, pagneNo, sortingValue, searchVal,this.candidateEmail).subscribe(
      (repsonsedata:ResponceData) => {
        if (repsonsedata.HttpStatusCode===200) {
          this.totalDataCount = repsonsedata.TotalRecord;
          this.listView = repsonsedata.Data;
  ...
  ngOnDestroy(): void {
    ...
    if (this.dataSubscription) {
      ...
      this.dataMailServiceSubscription.unsubscribe();
    }
  }
