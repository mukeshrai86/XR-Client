//  who:maneesh:what:ewm-13774 for create job action candidate add notes,when:05/09/2023
...
@Component({
  selector: 'app-job-action-add-job-notes',
  templateUrl: './job-action-add-job-notes.component.html',
  styleUrls: ['./job-action-add-job-notes.component.scss']
})
export class JobActionAddJobNotesComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<JobActionAddJobNotesComponent>, private appSettingsService: AppSettingsService, private fb: FormBuilder, private _KendoEditorImageUploaderService: KendoEditorImageUploaderService,
    ...
    this.jobActionNotesForm = this.fb.group({
      ...
      CategoryId: [''],
      //  who:maneesh,what:ewm-15142 for fixed validation message and required,when:20/11/2023
  ...
  ngOnInit(): void {
    ...
    this.RelatedTo.push({
      Id:this.JobId,UserType:this.jobCode,Name:this.JobTitle, 
  ...
  createNotes(value) {
    ...
    this.mentionList?.filter(item=>{
     ...
     let XRNotificationsArr:XRNotifications={
       ...
       NavigateUrl:"client/jobs/job/job-detail/detail?jobId="+this.candidatesid+'&workflowId='+this.WorkflowId+' &filter=All&selectjob=TotalJobs&tabIndex=3&uniqueId=',
       CompletePath:window.location.origin+"/"+"client/jobs/job/job-detail/detail?jobId="+this.candidatesid+'&workflowId='+this.WorkflowId+'&filter=All&selectjob=TotalJobs&tabIndex=3&uniqueId='
     };
    ...
    AddNotesObj={
      ...
      NotesContacts: this.selectedContactsItems?.map(item => ({
        'ContactId':item.Id,
...
} else if (data?.checked == false && type == 'cand' && this.candidatesid==Id) {
...
dropdownConfigContacts(){
  this.ContactsDropdownConfig = {
    API: this.serviceListClass.getContactByClientId +'?ClientId='+this.clientId,
    MANAGE: '',
    BINDBY: 'Name',
    REQUIRED: false,
    DISABLED: false,
  ...
  //  @Who: Renu, @When: 01-03-2024,@Why: EWM-16073 EWM-16256 @What:validation check  change on editor

  valueChange(value: string){
    if(value.replace(/(<([^>]+)>)/ig,"")?.length==0){
      this.jobActionNotesForm.get('Description').setErrors({required:true});
      this.jobActionNotesForm.get('Description').markAllAsTouched();
    }
  }

  
    //  @Who: Renu, @When: 05-03-2024,@Why: EWM-15213 EWM-16322 @What: on changes on kendo editor catch the event here  
    ...
    //  @Who: Renu, 05-03-2024,@Why: EWM-15213 EWM-16322 @What:get all mentions info from html tags on save/update
    ...
    //  @Who: Renu, @When: 05-03-2024,@Why: EWM-15213 EWM-16322 @What: get all event for editor here
    EditorEventCapture() {
