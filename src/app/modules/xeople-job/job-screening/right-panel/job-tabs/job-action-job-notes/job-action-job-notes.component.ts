//  who:maneesh:what:ewm-13774 for create job action candidate notes,when:05/09/2023
...
import { JobActionAddJobNotesComponent } from '../job-action-add-job-notes/job-action-add-job-notes.component';
import { GetNotesListModel } from 'src/app/shared/models/job-action-jobtab-note-list.model';
import { JobService } from 'src/app/modules/EWM.core/shared/services/Job/job.service';
import { ShortNameColorCode } from '@app/shared/models/background-color';


@Component({
  selector: 'app-job-action-job-notes',
  templateUrl: './job-action-job-notes.component.html', 
  styleUrls: ['./job-action-job-notes.component.scss']
})
export class JobActionJobNotesComponent implements OnInit {
...
//  who:maneesh:what:ewm-13870 for NotesList ,function:NotesList,when:24/08/2023
  NotesList(jobId: string, year: number, month: string, pageNo:number,searchValue:string) {
    ...
    this.loading = true;
        this.getNotesList=this.jobService.fetchJobNotesAll(jsonObj).subscribe(
        (data: ResponceData) => {
...
//  who:maneesh:what:ewm-13870 for openNoteListDataModel ,function:openNoteListDataModel,when:24/08/2023
  openNoteListDataModel() {
    ...
    const dialogRef = this.dialog.open(JobActionAddJobNotesComponent, {
      data: jsonObj,
