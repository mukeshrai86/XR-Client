...
@Component({
  selector: 'app-job-action-job-documents',
  templateUrl: './job-action-job-documents.component.html',
  styleUrls: ['./job-action-job-documents.component.scss']
})
export class JobActionJobDocumentsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.jobActionTabName?.toLowerCase()=='job'){
     this.getCandidateDocuments(this.jobId);
    }
  }
  /*
   @Type: File, <ts>
...
*/
  ...
  internalShareDocument() {
    let internalShareLink = '/client/jobs/job/job-detail/detail?jobId='+this.jobId+'&workflowId='+this.workflowId+ '&filter='+this.filter+'&selectjob='+this.selectjob+'&tabIndex='+5;
    const message = ``;
    const title = 'label_disabled';
    const subtitle = 'label_securitymfa';
    const dialogData = new ConfirmDialogModel(title, subtitle, message);
    const dialogRef = this.dialog.open(ShareDocumentComponent, {
      data: new Object({ documentData: this.documentData, candidateId: this.jobId, candidateName: this.JobTitle, jobId: this.jobId,shareData:true,internalShareLink:internalShareLink}),
  ...
  shareableLinkPopup(Id: number, DocumentName: string, documentType: string) {
    const dialogRef = this.dialog.open(DocumentShareableLinkComponent, {
      data: { candidateId: this.jobId, DocumentId: Id, documentType: documentType, jobId: this.jobId },
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      if (!dialogResult) {
        ...
        dialogResult.CandidateId = this.jobId;
        dialogResult.CandidateName = this.JobTitle;
 ...
 */
  openNewEmailModal(documentData: any) {
    ...
    const dialogRef = this.dialog.open(NewEmailComponent, {
      ...
      maxHeight: "100%",
      height: "100%",
      data: new Object({ documentData: documentData, candidateId: this.jobId, openDocumentPopUpFor: this.documentFor, isBulkEmail: true, JobId: this.jobId, IsJobLog: 1, DocumentId: this.documentId,workflowId:this.workflowId }),
...
*/
  ...
  */
  manageCandidateDocument() {
      let URL = '/client/jobs/job/job-detail/detail?jobId='+this.jobId+'&workflowId='+this.workflowId+ '&filter='+this.filter+'&selectjob='+this.selectjob+'&tabIndex='+5;
