...
@Component({
  selector: 'app-single-group-checklist',
  templateUrl: './single-group-checklist.component.html',
  styleUrls: ['./single-group-checklist.component.scss']
})
export class SingleGroupChecklistComponent implements OnInit {
  ...
  constructor(private jobService: JobService, private formBuilder: FormBuilder, private snackBService: SnackBarService,private translateService: TranslateService,public dialog: MatDialog,
    ...
    this._commonserviceService.checklistServiceObsObj.subscribe((res:any)=>{
      if (res) {
             this.checklistType = res;
            this.type= this.checklistType?.ChecklistType;
            this.checklistId = this.checklistType?.Id;
            this.getChecklistDetials(); 
          }
    });
   }
  ...
  /*
    @Name: getChecklistDetials function
    @Who: Nitin Bhati
    @When: 12-Sep-2023
    @Why: EWM-12599 EWM-13985
    @What: checklist info list
  */
  getChecklistDetials() {
    this.checkList = [];
    this.loading = true;
    if (this.checklistType?.ChecklistType == 'Single') {
      this.checkList = this.checklistType.ChecklistDetails.SingleCheckList;
      ...
      if (this.checklistType?.ChecklistDetails?.GroupCheckList) {
        this.checklistType?.ChecklistDetails?.GroupCheckList.forEach((element, i) => {
          element['CheckListQst']?.forEach((y, z) => {
            this.checkList?.push(y);
          });
          this.checkList?.forEach(element => {
            element['checked'] = false;
          });
        });
       this.checkListGroupName = this.checklistType?.ChecklistDetails?.GroupCheckList;
       this.checkListGroupName?.forEach(a => this.TotalMandatoryChecklistSum += a.TotalMandatoryChecklistCount);
       this.checkListGroupName?.forEach(a => this.TotalMandatoryPendingSum += a.TotalMandatoryChecklistPendingCount);
       }
     }
     this.loading = false;
  }
    ...
    if(this.checklistForm.value.checklistInfo?.length>0){
      this.checklistForm.value.checklistInfo?.forEach((x, index) => {
        parentobj.push({
          ...
          'ChecklistId': this.checklistType?.Id,
          'QstId': x.QstId,
          'TaskName': x.TaskName,
          'Comments': x.Comments,
          'Status': x.Status,
...
checkListOpen(list:any,isEditable:boolean,index:number){
  ...
  this._commonserviceService.groupchecklistServiceObs.next(list);
  }
  documentDataFetchFromChild(checkList:any){
    this.checkListStatus=false;
    this.panelOpenState = false;
    this.checkListGroupName.forEach(chkGroupList=>{
      let chkQstCount=chkGroupList.CheckListQst.length;
      let totalMandatoryChecklistCompleted=0;
      chkGroupList.CheckListQst.forEach(chkQst => {
         const chkListItem=this.checkList.find(chkList=>chkList.QstId==chkQst.QstId);
         if(chkListItem.Status==1 && chkQst.IsMandatory==1){
          totalMandatoryChecklistCompleted++;
         }
      });
      chkGroupList.TotalMandatoryChecklistPendingCount=(chkGroupList.TotalMandatoryChecklistCount as number)-(totalMandatoryChecklistCompleted as number);
    });
    this.savedChecklist.emit(this.checkList);
    }
...
*/
    openViewAttachmentModal(fileAttachments: any, rowIndex: number, type: string, groupCheckList: any) { 
      ...
      const dialogRef = this.dialog.open(ViewAttachmentPopupComponent, {
        maxWidth: "600px",
        data: new Object({
          fileType: this.fileType, fileSize: this.fileSize, fileSizetoShow: this.fileSizetoShow,fileAttachments: files
        }),
        width: "100%",
        maxHeight: "85%",
        panelClass: ['quick-modalbox', 'customAttachment', 'animate__animated', 'animate__zoomIn'],
    ...
    openDialogforDescription(rowIndex:number,item: any) {
      ...
      this.DescriptionResult.Source='SINGLE';
