...
@Component({
  selector: 'app-upload-document-checklist',
  templateUrl: './upload-document-checklist.component.html',
  styleUrls: ['./upload-document-checklist.component.scss']
})
export class UploadDocumentChecklistComponent implements OnInit {
 ...
 getDateFormat:any;
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any,private fb: FormBuilder, public dialogRef: MatDialogRef<UploadDocumentChecklistComponent>,
    ...
    this.dropDocumrntnameConfig['apiEndPoint'] = this.serviceListClass.getDocumentnameAllbyUsertype +'?userType='+ 'Job' +'&ByPassPaging=true&FilterParams.ColumnName=StatusName&FilterParams.ColumnType=Text&FilterParams.FilterValue=active&FilterParams.FilterOption=IsEqualTo&FilterParams.FilterCondition=AND';
  ...
  */
  ...
  onSave(value) {
    document.getElementsByClassName("add_folder")[0].classList.remove("animate__zoomIn");
      ...
      setTimeout(() => { this.dialogRef.close({res:true,'value':value,'fileInfo':this.fileInfo,'FileSize':this.fileSize}); }, 200);
  }
