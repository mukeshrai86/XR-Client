...
@Component({
  selector: 'app-push-candidate-success-poup',
  templateUrl: './push-candidate-success-poup.component.html',
  styleUrls: ['./push-candidate-success-poup.component.scss']
})
export class PushCandidateSuccessPoupComponent implements OnInit {
  candidatePushedInfo: any;
  httpstatus: number;
  message: string;

  constructor( public dialogRef: MatDialogRef<PushCandidateSuccessPoupComponent>,@Inject(MAT_DIALOG_DATA) public data: any,
  private pushCandidateEOHService: PushCandidateEOHService) { 
    this.candidatePushedInfo=data?.candidatePushedInfo;
    this.message=data?.candidatePushedInfo?.EOHResponseMsg;
