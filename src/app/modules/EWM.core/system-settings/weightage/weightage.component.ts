/*
  ...
  @When: 20-Oct-2021
  @Why: EWM-3431
  @What:  This page will be use for the Weightage configuration
*/
...
export class WeightageComponent implements OnInit {
  ...
  //  who:maneesh,what:ewm-10793 for default sorting  Weightage, change sortingValue,sortedcolumnName ,when:27/02/2023
  public sortingValue: string = "Weightage,asc";
  public sortedcolumnName: string = 'Weightage';
  ...
  public pageLabel: any = "label_weightage";
  client: any
  public idWeightage = '';
  public idName = 'Id';
  ...
  ngOnInit(): void {
    ...
    this.auditParameter = encodeURIComponent('Weightage Master');
...
*/
 ...
 @When: 20-Oct-2021
 @Why: EWM-3431
 @What: For showing the list of Weightage data
*/

  getWeightageList(pageSize, pageNo, sortingValue, searchVal, idName, idWeightage) {
    this.loading = true;
    this.getWeightageData = this._SystemSettingService.getWeightageList(pageSize, pageNo, sortingValue, searchVal, idName, idWeightage).subscribe(
  ...
  onScrollDown(ev?) {
    ...
    if (this.canLoad) {
      ...
      if (this.totalDataCount > this.gridViewList.length) {
        ...
        this.fetchWeightageMoreRecord(this.pageSize, this.pageNo, this.sortingValue);
      } else {
        this.loadingscroll = false;
      }
    } else {
 ...
 */
  ...
  fetchWeightageMoreRecord(pagesize, pagneNo, sortingValue) {
    this._SystemSettingService.getWeightageList(pagesize, pagneNo, sortingValue, this.searchVal, this.idName, this.idWeightage).subscribe(
      (repsonsedata: ResponceData) => {
  ...
  deleteWeightage(val, viewMode): void {
    ...
    const title = '';
    const subTitle = 'label_weightage';
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      if (dialogResult == true) {
        this._SystemSettingService.deleteWeightage(weightageObj).subscribe(
          (data: ResponceData) => {
...
*/
  ...
  //    }
  //    this.pageNo = 1;
  //    this._SystemSettingService.getWeightageList(this.pageSize, this.pageNo, this.sortingValue, this.searchVal, this.idName, this.idWeightage).subscribe(
  //      (repsonsedata: ResponceData) => {
  //        if (repsonsedata.HttpStatusCode === 200 || repsonsedata.HttpStatusCode === 204) {
  //          this.loading = false;
  //          this.loadingSearch = false;
...
@What: to unsubscribe the getWeightageData API 
*/
  ngOnDestroy(): void {
    this.getWeightageData.unsubscribe();
