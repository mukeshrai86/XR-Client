...
export class AccessRequestComponent implements OnInit {
  ...
  */
    constructor(private fb: FormBuilder,private systemSettingService:SystemSettingService,private snackBService:SnackBarService,
     public _sidebarService: SidebarService,private route: Router,private commonserviceService:CommonserviceService,private rtlLtrService:RtlLtrService,
    public dialog: MatDialog,private appSettingsService:AppSettingsService,private translateService: TranslateService) {
        ...
        sms_body:['',[Validators.required,Validators.maxLength(100),Validators.minLength(2)]],
        personTag:[''],
        jobTag:[''],
        id:[''],
      })
  ...
  */
    ngOnInit(): void {
     ...
     //
      ...
      this.userSmsList(this.pagesize, this.pagneNo, this.sortingValue,this.searchValue);
      setInterval(() => {
        this.canLoad = true;
        if (this.pendingLoad) {
          this.onScrollDown();
...
ngAfterViewInit(): void {
  ...
  */
 userSmsList(pagesize,pagneNo,sortingValue,searchVal)
 {
   this.loading=true;
   this.systemSettingService.fetchSmsList(pagesize,pagneNo,sortingValue,searchVal).subscribe(
     repsonsedata=>{     
...
*/
 userSmsListSearch(event: any)
 {
  ...
  this.systemSettingService.fetchSmsListSearch(this.searchChar).subscribe(
    ...
    this.systemSettingService.fetchSmsList(this.pagesize, this.pagneNo, this.sortingValue,this.searchValue).subscribe(
      repsonsedata=>{     
        this.listData=repsonsedata['Data'];
       },err=>{
        this.loading=false;
...
/*
 ...
 editForm(items) {
  this.addUserSmsForm.patchValue({
    id: items[0].Id,
    name: items[0].Title,
    sms_body: items[0].Sms_body,
  });
  this.smstemplateDescriptuion=items[0].Sms_body;
...
onScrollDown(ev?) {
  ...
  if (this.canLoad) {
    ...
    // Append elements /*  @Who: priti @When: 27-Apr-2021 @Why: EWM-1416 (add condition)*/
...
/* 
 ...
 {
  // this.loadingscroll=true;
   this.systemSettingService.fetchSmsList(pagesize,pagneNo,sortingValue,this.searchValue).subscribe(
     repsonsedata=>{    
       if(repsonsedata['HttpStatusCode']=='200')
       {
         this.loadingscroll=false;
