/*
  @(C): Entire Software
  @Type: File, <ts>
  @Name: lead-workflows-manage.component.ts
  @Who: Renu
  @When: 18-Dec-2024
  @Why: EWM-18938 EWM-18965.
  @What: lead-workflow manage
 */
...
import { MapChecklistComponent } from '../../job-workflows/map-checklist/map-checklist.component';
import { AddAssessmentComponent } from '../../job-workflows/add-assessment/add-assessment.component';
import { SidebarService } from '../../../../../shared/services/sidebar/sidebar.service';
import { LeadsService } from '../../../shared/services/leads/leads.service';
import { SystemSettingService } from '../../../shared/services/system-setting/system-setting.service';
export enum HiringStage {
  NewLead = 'New Lead',
  Closing = 'Convert to Client'
}
@Component({
  selector: 'app-lead-workflow-manage',
  templateUrl: './lead-workflow-manage.component.html'
})
export class LeadWorkflowManageComponent implements OnInit {

   /**********************All generic variables declarations for accessing any where inside functions********/
   public  newLead = HiringStage.NewLead;
   public Closing=HiringStage.Closing;
   ...
   // @ViewChildren(MatMenuTrigger) trigger: QueryList<MatMenuTrigger>;
    ...
    constructor(private fb: FormBuilder, private translateService: TranslateService, private router: ActivatedRoute,
      ...
      public _sidebarService: SidebarService,private quickJobService: QuickJobService,private leadsService:LeadsService,
      private systemSettingService:SystemSettingService) {
      this.JobWorkForm = this.fb.group({
        Id: [''],
        workflowName: ['', [Validators.required, Validators.maxLength(100),this.noWhitespaceValidator()]],
        ...
        if (this.tempID != undefined) {
          ...
          if(this.isClone){
            this.editForm(this.tempID);
             ...
             this.editForm(this.tempID);
            //this.CandidateLeadCount(this.tempID);
          }
        }
      }, 1500);
      this.industryList();
    ...
    */
    ...
    /*
    @Type: File, <ts>
    @Name: getClientTypeList function
    @Who: Renu
    @When: 30-Dec-2022
   ...
   */
      ...
      this.systemSettingService.getLocationTypeListAll('?FilterParams.ColumnName=StatusName&FilterParams.ColumnType=DropDown&FilterParams.FilterValue=active&FilterParams.FilterOption=IsEqualTo').subscribe(
        (repsonsedata: ResponceData) => {
          if (repsonsedata.HttpStatusCode === 200 ||repsonsedata.HttpStatusCode === 200) {
            this.loading = false;
            this.ClientTypeList = repsonsedata.Data;
    ...
    workflowExists(isSave) {
      ...
      if (this.JobWorkForm.get('workflowName')?.value.trim() == '') {
        return false;
      }
      let jsonExistObj = {};
      // jsonExistObj['Id'] = JobId;
      jsonExistObj['Id']= this.isClone && this.tempID ? '' : leadId;
    ...
    jsonExistObj['ClientTypeId']=this.JobWorkForm.get('clientType')?.value?this.JobWorkForm.get('clientType')?.value.InternalCode?.toString():'';
      if (this.JobWorkForm.get('workflowName')?.value) {
        this.leadsService.checkLeadWorkFlowIsExists(jsonExistObj).subscribe(
          (repsonsedata: any) => {
            if (repsonsedata.HttpStatusCode === 402) {
              if (repsonsedata.Status == false) {
                if(this.JobWorkForm.get('Industry')?.value==null && this.JobWorkForm.get('clientType')?.value==null){
               ...
               }else if(this.JobWorkForm.get('Industry')?.value!=null && this.JobWorkForm.get('clientType')?.value==null){
                ...
                //this.snackBService.showErrorSnackBar(this.translateService.instant('label_duplicateWorkflow'),'');
                this.JobWorkForm.get("workflowName")?.setErrors({ clientTypeTaken: true });
                this.JobWorkForm.get("workflowName")?.markAsTouched();
                this.JobWorkForm.get("workflowName")?.markAsDirty();
              }
              else if(this.JobWorkForm.get('Industry')?.value!=null && this.JobWorkForm.get('clientType')?.value!=null){
                //this.snackBService.showErrorSnackBar(this.translateService.instant('label_duplicateWorkflow'),'');
                this.JobWorkForm.get("workflowName")?.setErrors({ IndustryclientTypeTaken: true });
                this.JobWorkForm.get("workflowName")?.markAsTouched();
                this.JobWorkForm.get("workflowName")?.markAsDirty();
              }
              }
              ...
              if (repsonsedata.Status == true) {
                ...
                if (this.JobWorkForm && this.submitted == true && isSave) {
                  if (this.tempID == undefined || this.tempID == null) {
                    this.createLeadWorkFlow(this.JobWorkForm.getRawValue());
                  } else if(this.isClone){
                    this.createLeadWorkFlow(this.JobWorkForm.getRawValue());
                  }
                  else {
                    this.updateLeadWorkFlow(this.JobWorkForm.getRawValue());
                  }
                }
              }
            }
    ...
    */
    ...
    editForm(Id: Number) {
      ...
      this.loading = true;
      this.leadsService.getLeadWorkflowByID('?workflowId=' + Id).subscribe(
        (data: ResponceData) => {
          this.loading = false;
          if (data.HttpStatusCode === 200) {
            ...
            this.JobWorkForm.patchValue({
             ...
             // workflowName: this.dataArr.WorkflowName,
              ...
              Industry:this.dataArr?.IndustryId?{'Id':this.dataArr?.IndustryId,'Description':this.dataArr?.IndustryName}:[],
              clientType:this.dataArr?.ClientTypeId?{'InternalCode':this.dataArr?.ClientTypeId}:[],
            })
              // @ Adarsh singh EWM-9981 When: 30-12-2022
              if (this.isClone) {
                this.JobWorkForm.patchValue({
                  workflowName:'Clone-'+ this.dataArr.WorkflowName
                })
              }else{
                this.JobWorkForm.patchValue({
                  workflowName: this.dataArr.WorkflowName
            ...
            this.dataArr.Stages.forEach((x, index) => {
              this.hideChildContent[index]=true;
              ...
              let tempVarFirstStage:boolean[]=[];
              let tempVarLastStage:boolean[]=[];
              if(x['StageType']===HiringStage.NewLead){
                tempVarFirstStage[index]=true;
              }else if(x['StageType']===HiringStage.Closing){
                tempVarLastStage[index]=true;
              }
              parentInfo.push(
                this.fb.group({
                  ...
                  Active:new FormControl({
                    value:Boolean(Number(x['Status'] || x['StageType']==HiringStage.NewLead ||HiringStage.Closing))?Boolean(Number(x['Status'])):false,
                    disabled:Boolean(Number(x['Status'] &&  x['StageType']==HiringStage.NewLead||HiringStage.Closing))?true:false
                  }),
                  ...
                  JobPipeline:new FormControl(
                    {
                     ...
                     disabled:Boolean(Number(x['Status'] && x['StageType']==HiringStage.NewLead ||x['StageType']==HiringStage.Closing))?true:false
                    }),
    ...
    */
    ...
    /* 
     @Type: File, <ts>
     @Name: createLeadWorkFlow function
     @Who: Renu
     @When: 14-June-2021
     @Why: ROST-1872
    ...
    */
    ...
    createLeadWorkFlow(value) {
      this.loading = true;
      let JsonObj = {};
      JsonObj['Id'] = value.Id;
      JsonObj['workflowName'] = value.workflowName.trim();
      JsonObj['IndustryId']=value.Industry?.Id;
      JsonObj['IndustryName']=value.Industry?.Description;
      JsonObj['ClientTypeId']=value.clientType?.InternalCode;
      ...
      this.leadsService.AddLeadWorkflow(JsonObj).subscribe((repsonsedata: jobWorkFlow) => {
        if (repsonsedata.HttpStatusCode === 200) {
          this.loading = false;
          this.submitted = false;
          this.snackBService.showSuccessSnackBar(this.translateService.instant(repsonsedata.Message), repsonsedata.HttpStatusCode);
         
          this.route.navigate(['./client/core/administrators/lead-workflow']);
        } else if (repsonsedata.HttpStatusCode === 400) {
          this.snackBService.showErrorSnackBar(this.translateService.instant(repsonsedata.Message), repsonsedata.HttpStatusCode);
          this.loading = false;
        }
  ...
  */
    ...
    */
    stageExists(event, InternalCode, ParentStageId, formGroup: FormGroup) {
      ...
      this.leadsService.checkStageIsExists(jsonExistObj).subscribe(
        (repsonsedata: any) => {
          if (repsonsedata.HttpStatusCode === 200) {
            if (repsonsedata.Status == true) {
   ...
   */
    ...
    /* 
    @Type: File, <ts>
    @Name: updateLeadWorkFlow function
    @Who: Renu
    @When: 14-June-2021
    @Why: ROST-1872
   ...
   */
    ...
    updateLeadWorkFlow(value) {        
      ...
      JsonObj['IndustryName']=value.Industry?.Description;
      JsonObj['ClientTypeId']=value.clientType?.InternalCode;
      JsonObj['ClientType']=value.clientType?.Name;
      let parentobj = [];    
      ...
      this.leadsService.updateLeadWorkflow(JsonObj).subscribe((repsonsedata: jobWorkFlow) => {
        if (repsonsedata.HttpStatusCode === 200) {
          this.loading = false;
          this.submitted = false;
          this.snackBService.showSuccessSnackBar(this.translateService.instant(repsonsedata.Message), repsonsedata.HttpStatusCode);
          //this.route.navigate(['./client/core/administrators/lead-workflow'], { queryParams: { V: this.viewMode } });
          this.route.navigate(['./client/core/administrators/lead-workflow']);
        } else if (repsonsedata.HttpStatusCode === 400) {
          this.snackBService.showErrorSnackBar(this.translateService.instant(repsonsedata.Message), repsonsedata.HttpStatusCode);
          this.loading = false;
  ...
  */
    confirmDialogChildRow(pi: number, index: number, IsNew: number): void {
     ...
     /*  @Name: confirmDialogSubChildRow @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What:FOR DIALOG BOX confirmation for deletion of sub child*/
   ...
   /*  @Name: redirectTo @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What:redirect ot JOB page on back btn */
 ...
 /*  @Name: getStatusList @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What:For img url */
    ...
    /*  @Name: openAddAssessmentModule @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What: for open add assessment */
  ...
  /*  @Name: onChange @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What: For change color on chnage while select color */
    ...
    public onChange(getColor: string, stage: string, i, j, k): void {
      ...
      /*  @Name: changeActiveToggel @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What:  For change active status toggelworkflow data Sub-Child */
    ...
    /*  @Name: getGroupCheckListAll @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What:  For Group Checklist listing */
    ...
    closeToggel($event: any, tag, i, j, k) {
     ...
     /*  @Name: openMapChecklistModule @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What:  for open add checklist map  */
   ...
   /*  @Name: onCancelMapFeatures @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What: for close  menu  */
    ...
    onCancelMapFeatures(i){
     ...
     /*  @Name: onChangeFun @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What:for searching data while enter in input text */
    ...
    /*  @Name: getDeafultWorkflow @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What:for searching data while enter in input text */
  ...
  /*  @Name: getDeafultWorkflow @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What:getting the last and first stage of the workflow   */

    getDeafultWorkflow(){
      this.leadsService.getDefaultLeadWorkflow().subscribe(
        (data: ResponceData) => {
          if (data.HttpStatusCode === 200) {
            this.dataArr = data.Data;          
            //this.updateArr=data.Data;
              ...
              parentInfo.push(
                this.fb.group({
                 ...
                 // JobPipeline: new FormControl({ value: Boolean(Number(x['Status'])) == true ? Boolean(Number(x['IsIncludeInPipeline'])) : false, disabled: Boolean(Number(x['Status'])) ? false : true }),
                  ...
                  IsFirstStage:x['Code']==HiringStage.NewLead?1:0,
                  IsLastStage:x['Code']==HiringStage.Closing?1:0,
                  IsStageType:true
                })
              );
              if (this.mode == 'R') {
  ...
  /*  @Name: createSection @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What:when user click on add to create form group with same formcontrol   */
    ...
    /*  @Name: getWorkFlowStageList @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What:get workflowStage List   */

  getWorkFlowStageList(){
    this.leadsService.workflowapplicationstages().subscribe(
      (repsonsedata: ResponceData)  => {
        if (repsonsedata.HttpStatusCode ==200 || repsonsedata.HttpStatusCode ==204) {
          this.loading = false;         
  ...
  /*  @Name: onSlideStage @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What: on select stage type  */
   ...
   /*  @Name: updateStagesOnEdit @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What: update stages for first & last  on Edit  */
    ...
    */
    ...
    // }
        ...
        /*  @Name: checkStagesOnAdd @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What: check stages for first & last  on ADD  */

    checkStagesOnAdd(parentform,index:number){
      if(parentform.at(index).get('StageTypeCode').value===HiringStage.NewLead){
        this.fisrtStage=false;
        let indexA =  this.stagesTag.filter(x=>x['Code']===HiringStage.NewLead);
        if(indexA.length==0)
        {
          this.stagesTag.unshift(this.stagesTagCopy[0]);
        }
      }else if(parentform.at(index).get('StageTypeCode').value===HiringStage.Closing){
        this.lastStage=false;
        let indexH =  this.stagesTag.filter(x=>x['Code']===HiringStage.Closing);
        if(indexH.length==0)
        {
          this.stagesTag.push(this.stagesTagCopy[this.stagesTagCopy.length-1]);
        }
     ...
     /*  @Name: clearStageData @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What: for getting the formarray with this instance  */
   ...
   /*  @Name: stageTypePatch @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What:  create stage type list  */
    ...
    /*  @Name: stageInfo @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What: for getting the formarray with this instance */
    stageInfo() : FormArray {
      return this.stageForm.get("stageInfo") as FormArray
    }

/*  @Name: onCancel @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What: on dismiss the stage type popup  */
  ...
  /*  @Name: selectStageList @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What: on select stage type  */
  ...
  /*  @Name: selectTagList @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What:  selected stage list */
      ...
      onSaveStage(){
        ...
        this.stageForm.value.stageInfo.forEach(ele=> {
          if(ele.stageCheck==true){
            if(ele.Code===HiringStage.NewLead){
             // this.stagesTag.shift();
              this.fisrtStage=true;
              this.enableActiveJobPipeLine(parentform,true);
            }else if(ele.Code===HiringStage.Closing){
              //this.stagesTag.pop();
              this.lastStage=true;
              this.enableActiveJobPipeLine(parentform,true);
            }else{
   ...
   /*  @Name: enableActiveJobPipeLine @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What:enabling active status & job pipeline in case of first/last stage */
     ...
     /*  @Name: moveItemInArrayIfAllowed @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What: drop sections*/
   ...
   private moveItemInArrayIfAllowed(array: any[],fromIndex: number,toIndex: number): void {
    ...
    if (affectedItems.some((i) => (i.value.StageTypeCode==HiringStage.NewLead || i.value.StageTypeCode==HiringStage.Closing) ?true:false)) {
      return;
    }
    ...
    */
      ...
      compareFnClient(c1: any, c2: any): boolean {
        return c1 && c2 ? c1.InternalCode === c2.InternalCode : c1 === c2;
      }
  ...
  */
  ...
  /*  @Name: closeTemplate @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What:  close dropdown while click on more label */
  ...
  closeTemplate() {
   ...
   /*  @Name: onChaneColor @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What: selecting color on change */
  ...
  /*  @Name: CandidateLeadCount @Who: Renu  @When: 19-12-2024  @Why: EWM-18938 EWM-18965  @What: candidate job count to check workflow exists in other jobs or not */
    CandidateLeadCount(jobId:any){
    this.leadsService.CandidateLeadCount(jobId).subscribe(
      (data: ResponceData) => {
        if (data.HttpStatusCode === 200) {
          this.editForm(jobId);
          if(data.Data==true){
