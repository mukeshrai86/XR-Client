...
import { groupMaster, statusMaster } from '../../../shared/datamodels/status-master';
...
export class StatusComponent implements OnInit {
  ...
  public disabeldata:boolean=false;
  ...
  constructor(public dialog: MatDialog, private snackBService: SnackBarService, private router: Router,
    ...
    private translateService: TranslateService, private systemSettingService: SystemSettingService, private route: ActivatedRoute, private appSettingsService:AppSettingsService) {
    ...
    this.ApplicationStatusId = this._appSetting.ApplicationStatusId;
    this.groupCompanyCode = this._appSetting.groupCodeQuickClientDetailsPage;
    ...
    this.route.queryParams.subscribe(
      params => {
        if (params['groupId'] != undefined) {
          ...
          if(this.groupId===this.groupCompanyCode){
            this.StatusFor='All';
          }
          this.getGrupStatusList(this.pageSize, this.pageNo, this.sortingValue, this.searchVal, this.groupId,this.StatusFor);
        }
...
refreshComponent(){
  ...
  /*
 @Type: File, <ts>
 @Name: getGrupStatusList function
 @Who: Renu
 @When: 17-May-2021
 @Why: ROST-1538
 @What: For showing the list of group - status data
*/
  getGrupStatusList(pageSize, pageNo, sortingValue, searchVal, GroupId,StatusFor) {
    this.loading = true;
    this.systemSettingService.getStatusList(pageSize, pageNo, sortingValue, searchVal, GroupId,StatusFor).subscribe(
      (repsonsedata: statusMaster) => {   
        if (repsonsedata.HttpStatusCode === 200) {
          ...
          if (repsonsedata.Data) {
            this.gridViewList = repsonsedata.Data[0]['statuses'];
            this.groupCode = repsonsedata.Data[0]['GroupCode'];
            this.GroupId = repsonsedata.Data[0]['GroupId'];
            // this.reloadListData();
            // this.doNext();
            // <!-- who:maneesh,what:ewm-11486 for disabled edit btn when:31/05/2023 -->
            if (this.GroupId==this.ApplicationStatusId) {
              this.disabeldata=true;
            }else{
              this.disabeldata=false;
            }
          }
       ...
       //  who:maneesh,what:ewm-12630 for apply debounce when search data,when:06/06/2023
          ...
          // who:maneesh,what: for matching record not found toast desplay when no data so i m comment this,when:20/04/2023
  ...
  onScrollDown(ev?) {
    ...
    if (this.canLoad) {
      ...
      if (this.totalDataCount > this.gridViewList.length) {
        ...
        this.fetchStatusMoreRecord(this.pageSize, this.pageNo, this.sortingValue, this.groupId);
      } else {
        this.loadingscroll = false;
      }
    } else {
 ...
 */
  ...
  fetchStatusMoreRecord(pagesize, pagneNo, sortingValue, groupId) {
    this.systemSettingService.getStatusList(pagesize, pagneNo, sortingValue, this.searchVal, groupId,this.StatusFor).subscribe(
      (repsonsedata: statusMaster) => {
        if (repsonsedata.HttpStatusCode === 200) {
          this.loadingscroll = false;
          ...
          if (repsonsedata.Data) {
            ...
            nextgridView = repsonsedata.Data[0].statuses;
  ...
  confirmDialog(val, viewMode,index): void {
    ...
    const title = '';
    const subTitle = 'label_groupStatus';
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      if (dialogResult == true) {
        this.systemSettingService.deleteStatus('?GroupId=' + this.groupId + '&StatusId=' + val).subscribe(
          (data: statusMaster) => {
...
*/
 ...
 // comment this who:maneesh,what:ewm-12630 for apply debounce when search data,when:06/06/2023
  ...
  //   }
  //   this.pageNo = 1;
  //   this.systemSettingService.getStatusList(this.pageSize, this.pageNo, this.sortingValue, this.searchVal, this.groupId).subscribe(
  //     (repsonsedata: statusMaster) => {
  //       if (repsonsedata.HttpStatusCode === 200 || repsonsedata.HttpStatusCode === 204) {
  //         this.loading = false;
  //         this.loadingSearch = false;
  //         // who:maneesh,what:ewm-12071 for search data then message display no record found  this.gridViewList=repsonsedata.Data;when:20/04/2023
  ...
  */
  ...
  onSort(columnName) {
    ...
    this.pageNo = 1;
    this.systemSettingService.getStatusList(this.pageSize, this.pageNo, this.sortingValue, this.searchVal, this.groupId,this.StatusFor).subscribe(
      (repsonsedata: statusMaster) => {
        if (repsonsedata.HttpStatusCode === 200) {
