/*
  ...
  @When: 10-Dec-2021
  @Why: EWM-4140
  @What:  This page will be use for the Activity Category configuration
*/
...
export class ActivityCategoryComponent implements OnInit {
  ...
  // who:maneesh,what :change sortingValue  and sortedcolumnName  by default UserTypeName, why:ewm-10703 ewm-10790,when:24/02/2023
  ...
  public pageLabel: any = "label_activityCategory";
  client: any
  public idActivity = '';
  public idName = 'Id';
  ...
  activityCategoryData: Subscription;
  ...
  ngOnInit(): void {
    ...
    this.idActivity = decodeURIComponent(queryParams);
    if (this.idActivity == 'undefined') {
      this.idActivity = "";
    } else {
      this.idActivity = decodeURIComponent(queryParams);
    }
    ...
    this.auditParameter = encodeURIComponent('Activity Category');
    this.getActivitycategoryList(this.pageSize, this.pageNo, this.sortingValue, this.searchVal, this.idName, this.idActivity);
...
*/
  ...
  /*
   @Type: File, <ts>
   @Name: getActivitycategoryList function
   @Who: Nitin Bhati
   @When: 10-Dec-2021
  @Why: EWM-4140
   @What: For showing the list of Activity Category data
  */
  getActivitycategoryList(pageSize, pageNo, sortingValue, searchVal, idName, idActivity) {
    this.loading = true;
    this.activityCategoryData = this._SystemSettingService.getActivityCategoryList(pageSize, pageNo, sortingValue, searchVal, idName, idActivity).subscribe(
  ...
  */
  ...
  fetchMoreRecord(pagesize, pagneNo, sortingValue) {
    this._SystemSettingService.getActivityCategoryList(pagesize, pagneNo, sortingValue, this.searchVal, this.idName, this.idActivity).subscribe(
      (repsonsedata: ResponceData) => {
        if (repsonsedata.HttpStatusCode === 200) {
          this.loadingscroll = false;
  ...
  /*
      @Type: File, <ts>
      @Name: deleteActivityCategory function
      @Who: Nitin Bhati
      @When: 10-Dec-2021
      @Why: EWM-4140
      @What: FOR DIALOG BOX confirmation
    */
  deleteActivityCategory(val, viewMode): void {
    let weightageObj = {};
    weightageObj = val;
    const message = `label_titleDialogContent`;
    const title = '';
    const subTitle = 'label_activityCategory';
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      if (dialogResult == true) {
        this._SystemSettingService.deleteActivityCategory(weightageObj).subscribe(
          (data: ResponceData) => {
  ...
  */
  ...
  //   }
  //   this.pageNo = 1;
  //   this._SystemSettingService.getActivityCategoryList(this.pageSize, this.pageNo, this.sortingValue, this.searchVal, this.idName, this.idActivity).subscribe(
  //     (repsonsedata: ResponceData) => {
...
@What: to unsubscribe the activityCategoryData API 
*/
  ngOnDestroy(): void {
    this.activityCategoryData.unsubscribe();
