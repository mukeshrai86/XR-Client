...
@Component({
  selector: 'app-xeople-search-activity',
  templateUrl: './xeople-search-activity.component.html',
  styleUrls: ['./xeople-search-activity.component.scss']
})
export class XeopleSearchActivityComponent implements OnInit {
  ...
  selecteTimezone:any;
  timezoneArrList:any = [];
  @Input() selectedCandEmail: string;
  ...
  ngOnInit(): void {
    this.myActivityForm = this.fb.group({
      ...
      ActivityTitle: [this.Names, [Validators.required, Validators.maxLength(100)]],/** @When: 29-09-2023 @Who:maneesh for increas charecter length @Why: Ewm-14198 */
    ...
    // end 
      ...
      if(this.requiredAttendeesList?.length==0 && this.optionalAttendeesList?.length==0){
       ...
       this.requiredAttendeesList.push({
        Email: this.selectedCandEmail,
        Id: this.candidateIdData,
        Name: this.Names,
        Type: 'CAND'
      })
                       //  @Who: maneesh, @When: 09-04-2024,@Why: EWM-16681-EWM-16207 @What: on changes on kendo editor catch the event here
                       this.editorConfig={
                        REQUIRED:false,
                        DESC_VALUE:null,
      ...
      this.myActivityForm.get('IsSendEmailToAttendees').setValue(1); // maneesh discuss with shubhojit  ewm-17003 when:13/05/2024 
  }
...
// who : maneesh,when:09/12/2022 ,why :EWM.9598 what:  this.timePeriod='30';
 ...
 /* 
    @Type: File, <ts>
    @Name: getClientYearMonthList
    @Who: Nitin Bhati
    @When: 30-march-2023
    @Why: EWM-11527
    @What: For filter year month 
  */
 ...
 */
  ...
  onDismiss(): void {
    ...
    this.myActivityDrawerClose.emit({'isDrawerClose':true,'isSubmit':false}); //@Bantee drawer closer @whn 03-04-2023
    this.requiredAttendeesList = [];
    this.organizerOrAssigneesList = [];
    this.scheduleData = [];
    this.oldPatchValues = {};
      ...
      .subscribe(
        (data: ResponceData) => {
          if (data.HttpStatusCode === 200) {
            ...
            this.getEditorVal= res?.Description;//by maneesh
...
onchangeCategory(category) {
  if(this.selectedCategoryId==category?.Id){
   ...
   this.myActivityForm.patchValue({
     ...
     ActivityTitle: `${category?.ActivityCategory} ${this.Names ? '-' +this.Names : this.Names}`
   })
  }  
 }
  /*
  ...
  //<!---------@When: 30-03-2023 @who:Bantee @why: EWM-11165 --------->
...
*/
  previewEmailPopUp() {
    ...
    data: new Object({ subjectName: this.myActivityForm.get("ActivityTitle").value, emailTemplateData: this.myActivityForm.get("Description").value,RelatedUserType: this.category,RelatedUserId: this.candidateIdData,isActivity:true,JobId:this.JobId,openDocumentPopUpFor:"JOB"}),
