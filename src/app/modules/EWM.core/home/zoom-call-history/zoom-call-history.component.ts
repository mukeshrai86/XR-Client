...
import { CallHistroyComponent } from '@app/modules/EWM.core/home/call-histroy/call-histroy.component';
...
@Component({
  selector: 'app-zoom-call-history',
  templateUrl: './zoom-call-history.component.html',
  styleUrls: ['./zoom-call-history.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class ZoomCallHistoryComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog, private clientService: ClientService, private jobService: JobService,
    ...
    private candidateService: CandidateService, media: MediaMatcher, private route: Router,public _sidebarService: SidebarService,
  ) {
  ...
  OpenDocuemntPopUp(add) {
    const dialogRef = this.dialog.open(CallHistroyComponent, {
      data: new Object({
        ...
        ToNameredirectUrl: this.tONameRedirectUrl,
  ...
  openEditCallPopup(dataItem) {
    const dialogRef = this.dialog.open(CallHistroyComponent, {
      data: new Object({
        ShortName: this.ShortName, Name: this.candidateName, Id: this.candidateId, StatusColorCode: this.StatusColorCode,
        ImageUrl: this.ImageUrl, dataItem: dataItem, callId: dataItem?.callId, isEdit: 'Edit', usertype: "CAND",
      }),
      panelClass: ['xeople-modal', 'add-calllog', 'animate__animated', 'animate__zoomIn'],
      disableClose: true,
    });
  ...
  countVxtCall() {
   this.candidateService.countVxtCall("?id=" + '00000000-0000-0000-0000-000000000000' + "&usertype=" + '').pipe(takeUntil(this.destroy$)).subscribe(
     (repsonsedata: ResponceData) => {
       if (repsonsedata.HttpStatusCode === 200) {
         this.loading = false;
