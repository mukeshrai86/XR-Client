...
@Component({
  selector: 'app-workflow-sub-stages',
  templateUrl: './workflow-sub-stages.component.html',
  styleUrls: ['./workflow-sub-stages.component.scss']
})
export class WorkflowSubStagesComponent implements OnInit {
...
listData: any = [];
  ...
  constructor(public dialogRef: MatDialogRef<WorkflowSubStagesComponent>, private jobService: JobService,
    public dialog: MatDialog, @Inject(MAT_DIALOG_DATA) public data: any, private snackBService: SnackBarService,
    private translateService: TranslateService,private jobWorkflowService: JobService) {
...
*/
 getWorkFlowStagesAll(WorkFlowId: any) {
  this.loading = true;
  this.jobWorkflowService.getJobWorkflowByID('?workflowId=' + WorkFlowId).subscribe(
    (data: ResponceData) => {
      this.loading = false;
      if (data.HttpStatusCode === 200) {
