/*
  ...
  @What:  This page will be use for the Functional Sub Experties list Component ts file
*/
...
import { groupMaster } from 'src/app/modules/EWM.core/shared/datamodels/status-master';
import { DeleteConfirmationComponent } from 'src/app/shared/modal/confirm-dialog/delete-confirmation/delete-confirmation.component';
import { debounceTime } from 'rxjs/operators';

@Component({
  providers: [MessageService],
  selector: 'app-functional-sub-experties-list',
  templateUrl: './functional-sub-experties-list.component.html',
  styleUrls: ['./functional-sub-experties-list.component.scss'],
  animations: [
    trigger("flyInOut", [
...
export class FunctionalSubExpertiesListComponent implements OnInit {
  ...
  sortingValue: string = "FunctionalSubExpertise,asc";
  public sortedcolumnName: string = 'FunctionalSubExpertise';
  ...
  public ExpertiseId: any;
  public FunctionalExpertiseName: string;

  public userpreferences: Userpreferences;
  next: number = 0;
  listDataview: any[] = [];
  pageLabel:string='label_functionalExpertiesHead,label_functionalSubExpertiesHead'
  ...
  */
  constructor(private fb: FormBuilder, private commonServiesService: CommonServiesService, private systemSettingService: SystemSettingService, private snackBService: SnackBarService,
    ...
    this.auditParameter = encodeURIComponent('Functional Sub Expertise Master');
 ...
 */
  ngOnInit(): void {
    ...
    this.routes.queryParams.subscribe(
      (data) => {
        if (data['expertiesId'] != undefined) {
          this.ExpertiseId = data['expertiesId']
          localStorage.setItem('ExpertiseId', this.ExpertiseId);
          this.fetchfunctionalSubExpertiseList(this.pagesize, this.pagneNo, this.sortingValue, this.searchValue, this.ExpertiseId)
        }
        this.FunctionalExpertiseName = data['FunctionalExpertise'];
...
// refresh button onclick method by Piyush Singh
  ...
  */
  ...
  onAdd() {
    ...
    this.route.navigate(['/client/core/administrators/functional-experties/add-functional-sub-experties'], {
      queryParams: { ViewModeDataValue, ExpertiseId: this.ExpertiseId, FunctionalExpertise: this.FunctionalExpertiseName,
     }
    })
  }
  ...
  */
  onEdit(Id) {
    let ViewModeDataValue: any = this.viewMode
    this.route.navigate(['/client/core/administrators/functional-experties/add-functional-sub-experties', { id: Id }], {
      queryParams: { ViewModeDataValue, ExpertiseId: this.ExpertiseId }
    })
  }
  ...
  /* 
  @Type: File, <ts>
  @Name: fetchfunctionalSubExpertiseList function
   @Who: Anup
   @When: 18-June-2021
   @Why: EWM-1746 EWM-1843
  @What: service call for creating List data
  */
  fetchfunctionalSubExpertiseList(pagesize, pagneNo, sortingValue, searchVal, expertiseId) {
    this.loading = true;
    this.jobService.fetchfunctionalSubExpertiseList(pagesize, pagneNo, sortingValue, searchVal, expertiseId).subscribe(
      repsonsedata => {
        ...
        this.loadingSearch = false;
          ...
          // this.reloadListData();
          // this.doNext();
//  who:maneesh,what:ewm-12630 for apply debounce when search data and handel 204 ,when:08/06/2023
  ...
  */
  fetchfunctionalSubExpertiseListSearch(event: any) {
    ...
    if (numberOfCharacters < 1 || numberOfCharacters > 3) {
      if (numberOfCharacters > maxNumberOfCharacters) {
        ...
        this.listData = []
        this.jobService.fetchfunctionalSubExpertiseListSearch(this.searchChar, this.ExpertiseId).subscribe(
          repsonsedata => {
            this.loadingPopup = false;
            this.listData = repsonsedata['Data'];
            // this.reloadListData();
            // this.doNext();
            // this.fetchfunctionalSubExpertiseList(this.pagesize, this.pagneNo, this.sortingValue, this.searchValue)
        ...
        this.jobService.fetchfunctionalSubExpertiseList(this.pagesize, this.pagneNo, this.sortingValue, this.searchValue, this.ExpertiseId).subscribe(
          repsonsedata => {
            this.listData = repsonsedata['Data'];
            // this.fetchfunctionalSubExpertiseList(this.pagesize, this.pagneNo, this.sortingValue, this.searchValue)
            this.loadingPopup = false;
  ...
  /* 
  @Type: File, <ts>
  @Name: DeletefetchfunctionalSubExpertiseList function 
   @Who: Anup
   @When: 18-June-2021
   @Why: EWM-1746 EWM-1843
  ...
  */
  ...
  confirmDialog(val: any,index): void {
    ...
    const subtitle = 'label_functionalSubExperties';
    const title = '';
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      ...
      if (dialogResult == true) {
        ...
        this.jobService.deletefunctionalSubExpertiseById('?subExpertiseId=' + val).subscribe(
          repsonsedata => {
            this.active = false;
            this.loading = false;
            if (repsonsedata['HttpStatusCode'] == 200) {
  ...
  */
  ...
  onScrollDown(ev?) {
    ...
    if (this.canLoad) {
      ...
      if (this.totalDataCount > this.listData.length) {
        ...
        this.fetchfunctionalSubExpertiseListScroll(this.pagesize, this.pagneNo, this.sortingValue);
        // this.reloadListData();
        // this.doNext();
      }
  ...
  /* 
  @Type: File, <ts>
  @Name: fetchfunctionalSubExpertiseListScroll function
   @Who: Anup
   @When: 18-June-2021
   @Why: EWM-1746 EWM-1843
  @What: service call for listing sms list data on scroll
  */
  fetchfunctionalSubExpertiseListScroll(pagesize, pagneNo, sortingValue) {
    this.loadingscroll = true;
    this.jobService.fetchfunctionalSubExpertiseList(pagesize, pagneNo, sortingValue, this.searchValue, this.ExpertiseId).subscribe(
      repsonsedata => {
