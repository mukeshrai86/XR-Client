...
@Component({
  selector: 'app-reason-module',
  templateUrl: './reason-module.component.html',
  styleUrls: ['./reason-module.component.scss'],
  animations: [
    fadeInRightBigAnimation({ anchor: 'letterAnim4', duration: 500 }),
  ]
})
export class ReasonModuleComponent implements OnInit {
  loading: boolean;
  ...
  sortingValue: string = "ModuleName,asc";
  public sortedcolumnName: string = 'ModuleName';
  ...
  ngOnInit(): void {
    ...
    this.routes.params.subscribe((params) => {     
      this.ModuleName = params['can']; 
      this.jobService.ModuleNameObs.next(this.ModuleName);   
    });
    localStorage.setItem("moduleName",this.ModuleName);
    this.userpreferences = this._userpreferencesService.getuserpreferences();
    this.getAll(this.ModuleName, this.pagesize, this.pagneNo, this.sortingValue, this.searchValue, true, false);
...
mouseleaveAnimation(matIconId, animationName) {
  ...
  getAll(ModuleName, pagesize, pagneNo, sortingValue, searchVal, loader, loaderSerch) {
    this.loading = loader;
    this.loadingSearch = loaderSerch;
    this.jobService.getReasonGroupModule(ModuleName, pagesize, pagneNo, sortingValue, searchVal).subscribe(
      repsonsedata => {
        if (repsonsedata['HttpStatusCode'] == '200') {
          ...
          this.ModuleName = repsonsedata['Data'][0].ModuleName;
  ...
  onSearch(event: any) {
    ...
    if (numberOfCharacters < 1 || numberOfCharacters >= 3) {
      if (numberOfCharacters >= maxNumberOfCharacters) {
        this.getAll(this.ModuleName, this.pagesize, this.pagneNo, this.sortingValue, this.searchChar, false, true);
      } else {
        this.getAll(this.ModuleName, this.pagesize, this.pagneNo, this.sortingValue, this.searchChar, false, true);
      }
    }
  ...
  confirmDialog(val, index): void {
    ...
    deleteDataObj['Reason'] = val.Reason;
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      ...
      if (dialogResult == true) {
        this.loading = true;
        this.jobService.deleteReason(deleteDataObj).subscribe(
          repsonsedata => {
            this.active = false;
            this.loading = false;
            if (repsonsedata['HttpStatusCode'] == 200) {
  ...
  ListScroll(pagesize, pagneNo, sortingValue) {
    this.loadingscroll = true;
    this.jobService.getReasonGroupModule(this.ModuleName, pagesize, pagneNo, sortingValue, this.searchValue).subscribe(
      repsonsedata => {
        if (repsonsedata['HttpStatusCode'] == '200') {
          this.loadingscroll = false;
          let nextgridView = [];
  ...
  onView(data){
    this.jobService.ReasonObs.next({parentId:data.Id,parentName: data.GroupName,InternalCode:data.ReasonGroupInternalCode});
  }
}
