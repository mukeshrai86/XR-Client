...
export class CustomizationComponent implements OnInit {
  ...
  public less = "label_jobsettingless";
  ...
  constructor(private router: ActivatedRoute, private route: Router, private appSettingsService: AppSettingsService,public _sidebarService: SidebarService,
    ...
    private textChangeLngService:TextChangeLngService, private translateService: TranslateService) {
  ...
  renderMenuData(groupdata) {
    ...
    this.menuData.forEach(element => {
      if (element.Children.length != 0) {
        activeSubmenuChildren = this.findByMatchingProperties(element.Children, { Name: "customization" });
        ...
        if (activeSubmenuChildren.length != 0) {
          ...
          activeThirdLevel = this.findByMatchingProperties(activeSubmenuChildren[0]['Children'], { Name: "customization" });
        }
      }
    });
