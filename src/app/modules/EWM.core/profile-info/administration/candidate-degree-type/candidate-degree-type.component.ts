/*
  ...
  @When: 21-Aug-2021
  @Why: EWM-2501
  @What:  This page will be use for the degree type template Component ts file
*/
...
@Component({
  selector: 'app-candidate-degree-type',
  templateUrl: './candidate-degree-type.component.html',
  styleUrls: ['./candidate-degree-type.component.scss'],
  animations: [
    fadeInRightBigAnimation({ anchor: 'letterAnim4', duration: 500 }),
  ]
})
export class CandidateDegreeTypeComponent implements OnInit {
...
public sortedcolumnName: string = 'DegreeTypeName';
...
pageLabel: any = "label_degreeType";
...
searchSubject$ = new Subject<any>();
  getDegreeTypeData: Subscription;
...
constructor(public dialog: MatDialog, private snackBService: SnackBarService, private router: Router,
  ...
  private translateService: TranslateService, private _CandidateDegreeTypeService: CandidateDegreeTypeService, public _userpreferencesService: UserpreferencesService, private appSettingsService: AppSettingsService,) {
    // page option from config file 
  this.pageSize = this._appSetting.pagesize;
    this.pageOption = this._appSetting.pageOption;
  this.auditParameter = encodeURIComponent('Degree Type');
}
ngOnInit(): void {
  ...
  this.idDegreeType = decodeURIComponent(queryParams);
  if (this.idDegreeType == 'undefined') {
    this.idDegreeType = "";
  } else {
    this.idDegreeType = decodeURIComponent(queryParams);
  }
  ...
  this.getDegreeTypeList(this.pageSize, this.pageNo, this.sortingValue, this.searchVal, this.idName, this.idDegreeType);
...
/*
@Type: File, <ts>
@Name: getDegreeTypeList function
@Who: Nitin Bhati
@When: 21-Aug-2021
@Why: EWM-2502
@What: For showing the list of Degree Type data
*/
getDegreeTypeList(pageSize, pageNo, sortingValue, searchVal, idName, idExperience) {
  this.loading = true;
 this.getDegreeTypeData= this._CandidateDegreeTypeService.getDegreeTypeList(pageSize, pageNo, sortingValue, searchVal, idName, idExperience).subscribe(
    (repsonsedata: DegreeTypeMaster) => {
...
onScrollDown(ev?) {
  ...
  if (this.canLoad) {
    ...
    if (this.totalDataCount > this.gridView.length) {
      ...
      this.fetchDegreeTypeMoreRecord(this.pageSize, this.pageNo, this.sortingValue);
    }
    else {
      this.loadingscroll = false;
    }
...
/*
 @Type: File, <ts>
 @Name: fetchDegreeTypeMoreRecord
 @Who: Nitin Bhati
 @When: 21-Aug-2021
 @Why: EWM-2502 
 @What: To get more data from server on page scroll.
 */
fetchDegreeTypeMoreRecord(pagesize, pagneNo, sortingValue) {
   this._CandidateDegreeTypeService.getDegreeTypeList(pagesize, pagneNo, sortingValue, this.searchVal, this.idName, this.idDegreeType).subscribe(
    (repsonsedata: DegreeTypeMaster) => {
...
//   }
//   this.pageNo = 1;
//   this._CandidateDegreeTypeService.getDegreeTypeList(this.pageSize, this.pageNo, this.sortingValue, this.searchVal, this.idName, this.idDegreeType).subscribe(
//     (repsonsedata: DegreeTypeMaster) => {
//       if (repsonsedata.HttpStatusCode  == '200' || repsonsedata.HttpStatusCode == '204') {
//         this.loading = false;
//         this.loadingSearch = false;
...
DeleteInfo(degreeData: any): void {
  ...
  const title = '';
  const subTitle = 'label_degreeType';
  ...
  dialogRef.afterClosed().subscribe(dialogResult => {
    ...
    if (dialogResult == true) {
      this._CandidateDegreeTypeService.deleteDegreeType(degreeObj).subscribe(
...
@What: to unsubscribe the getDegreeTypeData API 
*/
ngOnDestroy(): void {
  this.getDegreeTypeData.unsubscribe();
