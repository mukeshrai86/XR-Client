/*
    ...
    @What:  This page is creted for Zoom Integration UI Component
*/
...
@Component({
  selector: 'app-zoom-call-integration',
  templateUrl: './zoom-call-integration.component.html',
  styleUrls: ['./zoom-call-integration.component.scss']
})
export class ZoomCallIntegrationComponent implements OnInit {
...
getIntegrationRegistrationByCode() {
  ...
  */
   EnableSave() {
    let currentUrl = window.location.origin + '/client/core/profile/other-integration/zoom-call-integration';
    this.router.snapshot.url;
    let responseType = this._appSetting.getResponseTypeZoom();  
    let state =currentUrl;
    let clientId = this._appSetting.getClientIdZoom();         
    let redirectUri = this._appSetting.getRedirectUriZoom(); 
 ...
 this.loading = true;
    ...
    */
      disconnectPopUp(): void {
        ...
        const title = '';
        let subTitle = '';
         if(this.IntegratorName=='Zoom Meeting Invite'){
          message = `label_titleDialogContentSiteDomain`;
          subTitle = 'label_disableZoomMeetingInvite';
        }else if(this.IntegratorName=='Zoom Phone Call'){
          message = ``;
