...
/*
    ...
    @Who: Nitin Bhati
    @When: 09-04-2024
    @Why: EWM-16214,EWM-16698
    @What: popup component for search notes by contacts
  */
...
@Component({
  selector: 'app-search-note-by-contact',
  templateUrl: './search-note-by-contact.component.html',
  styleUrls: ['./search-note-by-contact.component.scss']
})

export class SearchNoteByContactComponent implements OnInit {
  ...
  public selecteddata:[]
  clientId: string;
  constructor(public dialogRef: MatDialogRef<SearchNoteByContactComponent>, @Optional()
  @Inject(MAT_DIALOG_DATA) public data: any, public _dialog: MatDialog, private translateService: TranslateService,
    ...
    private snackBService: SnackBarService, private appSettingsService: AppSettingsService,private _clientService: ClientService) {
  ...
  onsearchTeammate(inputValue: string) {
    ...
    if (inputValue?.length > 3) {
      ...
      this._clientService.getClientNoteContactByClientid("?search="+inputValue+ "&ClientId="+this.clientId).subscribe(
        (repsonsedata: any) => {
