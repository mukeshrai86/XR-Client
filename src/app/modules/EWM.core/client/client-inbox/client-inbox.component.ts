 ...
 */
  openNewEmailModal(responseData: any, mailRespondType: string) {
    ...
    const dialogRef = this.dialog.open(ClientNewEmailComponent, {
      ...
      maxHeight: "100%",
      height: "100%",
      data: { 'res': responseData, 'mailRespondType': mailRespondType, 'openType': this.emailConnection, 'candidateMail': this.clientEmailId,
 ...
 */
  ...
  fetchInboxList(pagesize, pagneNo, sortingValue, searchVal, Imp) { 
    ...
    this.mailService.fetchClientMailList(pagesize, pagneNo, sortingValue, searchVal, this.clientEmailId,this.clientId).subscribe(
      (repsonsedata: ResponceData) => {
        if (repsonsedata.HttpStatusCode === 200) {
          this.loading = false;
          this.totalDataCount = repsonsedata.TotalRecord;
 ...
 */
  getMailCount() {
    this.loading=true;
    this.mailService.fetchMailClientCount(this.clientId).subscribe(
      (repsonsedata: ResponceData) => {
        if (repsonsedata.HttpStatusCode === 200) {
          //this.loading = false;
