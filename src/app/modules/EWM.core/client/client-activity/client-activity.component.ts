/*
  ...
  @What:  This page will be use for Activity for client section
*/
...
export class ClientActivityComponent implements OnInit {
 ...
 public plcData = [];
  ...
  constructor(changeDetectorRef: ChangeDetectorRef, media: MediaMatcher, private fb: FormBuilder, private routes: ActivatedRoute, public _sidebarService: SidebarService, private commonServiesService: CommonServiesService,
    ...
    private dataService: CommonDropDownService, private cache: CacheServiceService,) {
  ...
  ngOnInit(): void {
   ...
   this.myActivityForm = this.fb.group({
    ...
    ActivityTitle: [this.Names, [Validators.required, Validators.maxLength(500)]],/** @When: 28-11-2023 @Who:maneesh for increes charecter length @Why: Ewm-15173 */
 ...
 */
  ...
  clearFilterData(viewMode: string): void {
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      ...
      this.searchData='disableOwwnerFilterIcon';
      if (dialogResult == true) {
        ...
        if (viewMode == 'Owner') {
          ...
          this.selectedorDeselectedOwner=[];//who:maneesh,what:ewm-18937,when:24/01/2024
   ...
   this.requiredAttendeesList.forEach(element=>{
     if (element.Type == "External") {
       ...
       requiredAttendeesList.push({
         ...
         Mode: element?.Mode,
         Type: element?.Type ? element?.Type : 'CLIE'
       })
     }
   })
  ...
  openDialogJobCategory() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result!=false ) {
        ...
        this.filterCountOwner = this.OwnerIds?.length;
        this.selectedOrDeSelected = result?.selectedOrDeSelected;//who:maneesh,what:ewm-18937 for patch data when edit search catogry
        if (result?.selectedOrDeSelected!=undefined) {
        let selectedArray = result?.selectedOrDeSelected?.filter(x => x?.IsSelected == 1); //who:maneesh,what:ewm-18937 for patch data when edit search catogry
...
formType(value) {
  ...
  if (this.activestatus != 'Edit' &&  this.Names) {
    let enc:string = btoa('selectedCand');
    let email = localStorage.getItem(enc);

    this.requiredAttendeesList.push({
      Email: this.clientEmailId,
...
*/
previewEmailPopUp() {
  if (this.myActivityForm.get("Description").value==null) {
...
dropdownConfig(){
  ...
  // who:maneesh,what: get editor value data,when:02/04/2024
