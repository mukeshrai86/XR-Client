...
@Component({
  selector: 'app-contact-call-histroy',
  templateUrl: './contact-call-histroy.component.html',
  styleUrls: ['./contact-call-histroy.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class ContactCallHistroyComponent implements OnInit {
  ...
  // public sizes = [];
  GridId = "CallHistoryContactGrid_001";
  ...
  ContactCallcontactId: any;
  ContactCallContactIdName: string;
  contactIdForNUmberTyp: string;
  ...
  constructor(public dialog: MatDialog, private clientService: ClientService, private jobService: JobService,
    ...
    this.ContactCallcontactId = sessionStorage.getItem('ContactCallcontactId');  
    this.ContactCallContactIdName = sessionStorage.getItem('ContactCallContactIdName'); 
    this.ShortName = sessionStorage.getItem('ContactCallShortName');     

    this.candidateId = this.ContactCallcontactId;
    this.getCallLogData = JSON.parse(sessionStorage.getItem('ContactCallLogDatass'));    
    this.candidateName = this.ContactCallContactIdName;
    this.contactIdForNUmberTyp= localStorage.getItem('contactIdForNUmberTyp'); 
    this.ImageUrl = this.getCallLogData?.ProfileImageURL;
    this.PhoneNumber = this.getCallLogData?.PhoneNumber;
    this.emailId=this.getCallLogData?.EmailId;
    this.ProfileImagePath=this.getCallLogData?.ProfileImagePath;
  ...
  ngOnInit(): void {
    this.commonserviceService?.getcontactsmsHistroy?.subscribe(res => {
      if (res) {
        ...
        this.ImageUrl = res?.ProfileImageURL;
        this.commonserviceService.countDataRefreshForContact.next({contact:true,contactId:this.candidateId});
      }     
     })
  ...
  public sendRequest(state: State): void {
    this.redirectContactId= localStorage.getItem('redirectContactId');
    // if (this.redirectContactId!=null && this.redirectContactId!=undefined) { //by maneesh
    // this.candidateId =this.redirectContactId; 
    // }
    this.animate();
  ...
  OpenDocuemntPopUp(add) {    
    const dialogRef = this.dialog.open(CommonAddCallLogComponent, {
      data: new Object({
        ...
        ImageUrl: this.ImageUrl, usertype: "CONT", isEdit: add, gridlistData: this.gridListData,
        ToNameredirectUrl: this.tONameRedirectUrl,reletedUserTypeCodeValue:'Contact',reletedUserTypeCode:'CONT',
        FromNameredirectUrl: this.fromRedirectUrl, detailsSummeryUrl: this.detailsSummeryUrl,contactIdForNUmberTyp:this.contactIdForNUmberTyp,
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      ...
      this.commonserviceService.countDataRefreshForContact.next({contact:true,contactId:this.ContactCallcontactId});
    });
    if (this.appSettingsService.isBlurredOn) {
      document.getElementById("main-comp").classList.add("is-blurred");
    }
  ...
  openEditCallPopup(dataItem) {
    const dialogRef = this.dialog.open(CommonAddCallLogComponent, {
      data: new Object({
        ShortName: this.ShortName, Name: this.candidateName, Id: this.candidateId, StatusColorCode: this.StatusColorCode,
        ImageUrl: this.ImageUrl, dataItem: dataItem, callId: dataItem?.callId, isEdit: 'Edit', usertype: "CONT",
        reletedUserTypeCodeValue:'Contact',reletedUserTypeCode:'CONT',contactIdForNUmberTyp:this.contactIdForNUmberTyp,
        PhoneNumber:this.PhoneNumber,EmailId:this.emailId,ProfileImagePath:this.ProfileImagePath
      }),
      panelClass: ['xeople-modal', 'add-calllog', 'animate__animated', 'animate__zoomIn'],
      disableClose: true,
