...
@Component({
  selector: 'app-grid-tree-view',
  templateUrl: './grid-tree-view.component.html',
  styleUrls: ['./grid-tree-view.component.scss'],
  animations: [
    fadeInRightBigAnimation({ anchor: 'letterAnim4', duration: 500 }),
  ]
})

export class GridTreeViewComponent implements OnInit {
  @Input() opened = false;
...
delaAnimation(i:number){
  ...
  OpenVersionPopUp(documentData) {
    const dialogRef = this.dialog.open(VersionComponent, {
      ...
      panelClass: ['xeople-modal-lg', 'add_version', 'animate__animated', 'animate__zoomIn'],
      disableClose: true,
    });
    dialogRef.afterClosed().subscribe(dialogResult => {
      if(dialogResult) {}
