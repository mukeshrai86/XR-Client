...
export class CandidateDocumentComponent implements OnInit {
  ...
  constructor( public _userpreferencesService: UserpreferencesService,public dialog: MatDialog, public candidateService:CandidateService,
    ...
    ,private appSettingsService: AppSettingsService, private commonserviceService: CommonserviceService,) {
       /*
        @Type: File, <ts>
        @Name: Json file 
        @Who: Nitin Bhati
...
//maneesh fixed scroll  when:07/05/2024 what:16133
  onScrollDown() {
    this.loadingscroll = true;
    if (this.canLoad) {
      this.canLoad = false;
      this.loadingscroll = true; 
      if (this.totalRecords > this.dataToBind?.length) {
   ...
   .subscribe((data:ResponceData)=>{
     if(data.HttpStatusCode==200)
     {
      ...
      if(isScroll) { 
        ...
        let dataToBindJson=this.gridView;
...
restoreExpandedNodes() {
 ...
 /*
  ...
  refreshComponent(){
    ...
    this.canLoad=true;
    this.searchVal='';
    this.pageNo=1;    //by maneesh ewm-16133 when:14/05/2024
    this.getAlldocument(false,false);
  }
// filterDocById(){
