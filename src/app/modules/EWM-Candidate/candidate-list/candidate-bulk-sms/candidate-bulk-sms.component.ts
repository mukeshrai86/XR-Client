...
@Component({
  selector: 'app-candidate-bulk-sms',
  templateUrl: './candidate-bulk-sms.component.html',
  styleUrls: ['./candidate-bulk-sms.component.scss']
})
export class CandidateBulkSmsComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any, private fb: FormBuilder, private jobService: JobService,
    ...
    public dialogRef: MatDialogRef<CandidateBulkSmsComponent>, private commonserviceService: CommonserviceService, public dialog: MatDialog, private translateService: TranslateService,
  ...
  ngOnInit(): void {
    ...
    // const arrayOfObjectsAllContacts = this.data?.selectedCandidates;
    //     this.selectedCandidates = arrayOfObjectsAllContacts.filter((obj, index, self) =>
    ...
    this.mappedPhones =  this.selectedCandidates?.filter(res=>res?.PhoneNumber!='' && res?.PhoneNumber!=null && res?.PhoneNumber!=undefined);;
...
*/
  onConfirm(value) {
    ...
    this.mappedPhones?.forEach(element => {
      ...
      ToPhone["Name"] = element?.Name,
      ToPhone["ToPhone"] = '+'+element?.PhoneCode+''+element?.PhoneNumber,
