...
export class CandidateSmsComponent implements OnInit {
 ...
 public SMSHistory : any =[];
  ...
  @Input() JobDetailCandidateId:string;
  ...
  if (!this.candidateId) { //by maneesh ewm-
    this.candidateId=this.JobDetailCandidateId;
  }
  this.getSMSHistory();
...
getSMSHistory() {
  ...
  this.loading = true;
  let userType='CAND';
  this.systemSettingService.getSMSHistory('?Id='+this.candidateId+'&UserType='+userType).subscribe(
    (repsonsedata: ResponceData) => {
      this.loading = false;
      if (repsonsedata.HttpStatusCode === 200) {
        this.SMSHistory = repsonsedata.Data;
        this.loading = false;
        if(this.candidateIdData!=undefined){ //by maneesh ewm-16656 when:29/04/2024
          setTimeout(() => {
            this.myScrollContainer.nativeElement.scroll({
              top: this.myScrollContainer.nativeElement.scrollHeight,
              left: 0,
