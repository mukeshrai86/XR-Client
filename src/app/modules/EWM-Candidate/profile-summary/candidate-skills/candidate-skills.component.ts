 ...
 */
  ...
  */
  ...
  addSkills() {
    this.loading = true;
    let SkillsJsonObj = {};
    SkillsJsonObj['CandidateId'] = this.candidateId;
    SkillsJsonObj['CandidateName'] = this.candidateName;
    SkillsJsonObj['Skills'] =  this.skillSelectedList.filter((thing, index, self) =>
    index === self.findIndex((t) => (
      t.Id === thing.Id
    ))
  );//this.Skills;
    this.candidateService.addcanSkillsData(SkillsJsonObj).subscribe(
      (repsonsedata: any) => {
        if (repsonsedata.HttpStatusCode === 200) {
          this.loading = false;
          // this.getAllSkills();
...
*/
// @When: 05-08-2024 @who:Amit & Maneesh @why: EWM-17817 @what: skill count & tag lable show
mintitle:string;
  openPopupForSkillCountOfTag(count, SkillDataByTag, tagName): void {
    ...
    // who:maneesh,what:EWM.9083 apply if else condition,when:05-12-2022
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      if (dialogResult == true) {
        for (let index = 0; index < SkillDataByTag.length; index++) {          
          ...
          console.log(this.skillSelectedListId,"this.skillSelectedListId");
        }
    ...
    this.systemSettingService.fetchSkillsByTagId("?skilltagid=" + id).subscribe(
      (repsonsedata: any) => {
        if (repsonsedata.HttpStatusCode === 200 || repsonsedata.HttpStatusCode === 204) {
          ...
          // who:maneesh,what:ewm.10109 this.skillsListByTag==null and undefine,when:06/02/2023
          if (this.skillsListByTag==null ||this.skillsListByTag==undefined) {
            this.isSkill=true;
          }else{
            this.isSkill=false;
          }
          let SkillCount = repsonsedata.SkillCount;
  ...
  public selectedSkills(eventData): void {
    ...
    if (event?.IsTag === 0) {
      if (this.skillSelectedList.some(el => el.SkillName == event?.SkillName)) {
        ...
        // var notSelected = this.searchskillList.filter(x => this.skillSelectedListId.findIndex(y => y.Id == x.Id) == -1);
        // var x = [...this.skillSelectedListId, ...notSelected];
        // this.searchskillList = [...x];
...
rearrangeSkills(){
  var notSelected = this.originalSearchskillList.filter(x => this.skillSelectedListId.findIndex(y => y.Id == x.Id) == -1);
        var x = [...this.skillSelectedListId, ...notSelected];
        this.searchskillList = [...x];
    ...
    if (inputValue.length > 0 && inputValue.length > 1) {
      ...
      this.quickJobService.getAllSkillAndTag("?Search=" + inputValue +'&FilterParams.ColumnName=Status&FilterParams.ColumnType=Text&FilterParams.FilterValue=active&FilterParams.FilterOption=IsEqualTo&FilterParams.FilterCondition=AND&ByPassPaging=true').subscribe(
        (repsonsedata: any) => {
          if (repsonsedata.HttpStatusCode === 200 || repsonsedata.HttpStatusCode === 204) {
            this.searchskillList = repsonsedata.Data;
            this.originalSearchskillList = [...this.searchskillList];
  ...
  */
  ...
  /*
   @Type: File, <ts>
   @Name: onClickcloseOnSelect function
   @Who: Nitin Bhati
   @When: 27-July-2022
   @Why: EWM-15617
   @What: User should be able to select multiple skills from drop dow
   */
   onClickcloseOnSelect(item){
    if(item === 0){
    this.closeOnSelectSkill=false;
    }else{
      this.closeOnSelectSkill=true;
    }
  }
}
