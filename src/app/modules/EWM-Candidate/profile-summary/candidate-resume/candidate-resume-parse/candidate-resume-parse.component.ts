...
@Component({
  selector: 'app-candidate-resume-parse',
  templateUrl: './candidate-resume-parse.component.html',
  styleUrls: ['./candidate-resume-parse.component.scss']
})
export class CandidateResumeParseComponent implements OnInit {
    ...
    this.candidateService.parseResume(FileObj).subscribe(
      (repsonsedata: ResponceData) => {
        if (repsonsedata.HttpStatusCode === 200) {
          ...
          this.canProfileData = this.parseResumeData?.ProfileDetails;
          this.candidateResumeData = this.parseResumeData;
          // this.canEducationData = this.parseResumeData?.EducationDetails;
          // this.canEmploymentData = this.parseResumeData?.EmploymentHistoryDetails;
          // this.canExperienceData = this.parseResumeData?.ExperienceDetails;
          // this.canSkillsData = this.parseResumeData?.SkillDetails;
          this.generalLoader = false;
          //this.getResumeByCandidateId();
        } else if (repsonsedata.HttpStatusCode === 204) {
  ...
  */
  openUploadResumeBox() {
    const dialogRef = this.dialog.open(UploadNewResumeComponent, {
      ...
      panelClass: ['uploadNewResume', 'animate__slow', 'animate__slow', 'animate__animated', 'animate__fadeInDownBig'],
      disableClose: true,
    });
...
highligthSearchText(textToSearch){
  ...
  let pattern  = textToSearch.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g,'\\$&');   
  pattern=pattern
        .split(' ')
        .filter(t => t)
        .join('|');
  ...
  for(var x = 0; x<documentAll?.length; x++){
    if(documentAll[x].innerHTML.toLowerCase().match(pattern.toLowerCase())){      
      documentAll[x].innerHTML = documentAll[x].textContent.replace(regex, match => `<span class="highlight"><mark>${match}</mark></span>`);
